...
export class HanaEngine {
  ...
  constructor({ hanakoHome, productDir, agentId }) {
    ...
    this._cleanupRemovedExternalMcpServers();
  ...
  _cleanupRemovedExternalMcpServers() {
    const REMOVED_SERVER_KEYS = ["claude_in_chrome"];
    ...
    if (mcp._claude_in_chrome_external_migrated !== undefined) {
      delete mcp._claude_in_chrome_external_migrated;
      changed = true;
    }
    if (!changed) return;
