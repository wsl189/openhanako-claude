...
/** 内容块是否为工具调用（兼容 tool_use / toolCall 两种格式） */
...
export async function summarizeTitle(utilConfig, userText, assistantText) {
  try {
    ...
    const isZh = getLocale().startsWith("zh");
    // 推理型模型常先产出 thinking，再给最终正文。
    // 标题任务统一给足预算，避免在“思考阶段”被截断导致取不到最终标题。
    const titleMaxTokens = 1000;
...
5. Output the title directly, no explanation`;
    ...
    const cleaned = String(raw || "")
      ...
      .replace(/<think\b[^>]*>[\s\S]*?<\/\s*think\s*>/gi, " ")
      .replace(/<\/?\s*think\b[^>]*>/gi, " ")
      .replace(/\s+\n/g, "\n")
      .replace(/\n{3,}/g, "\n\n")
      .replace(/[ \t]+/g, " ")
