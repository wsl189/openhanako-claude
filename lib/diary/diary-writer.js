...
function collectActivities(store, rangeStart, rangeEnd) {
  ...
  return entries.map(e => {
    ...
    const time = normalizeClockLabel(new Date(e.startedAt).toLocaleTimeString(locale, {
      hour: "2-digit",
      minute: "2-digit",
      hour12: false,
    }));
...
/**
 * 从 session JSONL 读取消息（用于 /diary 的兜底摘要）
 * @param {string} sessionPath
 * @returns {Array<{role: "user"|"assistant", content: any, timestamp?: string|null}>}
 */
 ...
 * @param {string} opts.agentPersonality - agent 的人格 prompt（identity + ishiki）
 * @param {string} opts.memory - agent 的 memory.md 内容
 * @param {string} opts.userName
 * @param {string} opts.agentName
 * @param {string} opts.cwd - 工作空间目录路径
 * @param {string} [opts.currentSessionPath] - 当前会话文件路径（用于兜底摘要）
...
export async function writeDiary(opts) {
  const {
    ...
    cwd, currentSessionPath, activityStore,
  } = opts;
  ...
  // 兜底：若当天摘要为空，尝试直接从当前 session 生成一份摘要（即使记忆开关关闭）
  if (summaries.length === 0 && currentSessionPath) {
    try {
      const msgs = readSessionMessages(currentSessionPath);
      if (msgs.length > 0) {
        const sessionId = path.basename(currentSessionPath).replace(/\.jsonl$/, "");
        await summaryManager.rollingSummary(sessionId, msgs, resolvedModel);
        summaries = summaryManager.getSummariesInRange(rangeStart, rangeEnd);
      }
    } catch (err) {
      console.warn(`[diary] fallback summary failed: ${err.message}`);
    }
  }
  ...
  if (summaries.length === 0) {
    ...
    return {
      error: isZh
        ? "今天还没有可用对话摘要，没什么可写的（可先聊几句再试）"
        : "No usable conversation summaries today — try again after chatting a bit.",
    };
  }
  ...
  diaryContent = String(diaryContent || "").trim();
  ...
  // 6. 文件名：日期(YYYY.M.D) + 时间 + agent 名（不再拼接标题，避免日期冗余）
  const now = new Date();
  const hh = String(now.getHours()).padStart(2, "0");
  const mm = String(now.getMinutes()).padStart(2, "0");
  const [yy, mRaw = "1", dRaw = "1"] = String(logicalDate).split("-");
  const fileDateLabel = `${yy}.${Number(mRaw)}.${Number(dRaw)}`;
  const safeAgentName = String(agentName || "agent")
    .replace(/[/\\:*?"<>|]/g, "")
    .trim()
    .slice(0, 40) || "agent";
  // 时间格式固定为 HH:MM（冒号前后不含空格）
  const timeLabel = normalizeClockLabel(`${hh}:${mm}`);
  let fileName = `${fileDateLabel} ${timeLabel} ${safeAgentName}.md`;
  ...
  if (fs.existsSync(filePath)) {
    let i = 2;
    while (true) {
      const candidate = `${fileDateLabel} ${timeLabel} ${safeAgentName} (${i}).md`;
      const candidatePath = path.join(diaryDir, candidate);
