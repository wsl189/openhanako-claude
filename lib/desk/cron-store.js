...
function resolveCronNotifyPolicy(notifyTarget, notifyPlatform, fallbackTarget = "local") {
  const normalizedNotifyPlatform = normalizeNotifyPlatform(notifyPlatform);
  let normalizedNotifyTarget = normalizeNotifyTarget(notifyTarget, fallbackTarget);
  if (normalizedNotifyPlatform) {
    normalizedNotifyTarget = "platform";
  } else if (normalizedNotifyTarget === "auto") {
    // 产品约定：未指定平台时，定时提醒默认走本地弹窗。
    normalizedNotifyTarget = "local";
  }
  return { normalizedNotifyTarget, normalizedNotifyPlatform };
}
...
export class CronStore {
  ...
  _load() {
    try {
      ...
      this._jobs = (Array.isArray(data.jobs) ? data.jobs : []).map((job) => {
        const { normalizedNotifyTarget, normalizedNotifyPlatform } = resolveCronNotifyPolicy(
          job?.notifyTarget,
          job?.notifyPlatform,
          "local",
        );
   ...
   * @param {"local"|"platform"|"auto"} [opts.notifyTarget="local"] - 提醒通知目标
   * @param {"wechat"|"telegram"|"feishu"|"qq"|""} [opts.notifyPlatform=""] - 指定提醒平台
   * @returns {object} 新建的 job
   */
  addJob({ type, schedule, prompt, mode = "isolated", label = "", model = "", notifyTarget = "local", notifyPlatform = "" }) {
  ...
  updateJob(id, partial) {
    ...
    const { normalizedNotifyTarget, normalizedNotifyPlatform } = resolveCronNotifyPolicy(
      merged.notifyTarget,
      merged.notifyPlatform,
      "local",
    );
    merged.notifyPlatform = normalizedNotifyPlatform;
    merged.notifyTarget = normalizedNotifyTarget;
