...
export function createCronScheduler({ cronStore, executeJob, abortJob, onJobDone }) {
  // 默认 10 秒检查一次，减少定时提醒体感延迟（可用环境变量覆盖）。
  const envMs = Number(process.env.HANA_CRON_CHECK_MS);
  const CHECK_INTERVAL = Number.isFinite(envMs) && envMs >= 1_000
    ? Math.trunc(envMs)
    : 10_000;
  let _timer = null;
  let _checking = false;
  ...
  async function _doCheck() {
    try {
      ...
      for (const job of jobs) {
        ...
        const EXEC_TIMEOUT = 30 * 60 * 1000; // 30 分钟超时
        try {
          {
            let timer;
            try {
              await Promise.race([
                ...
                new Promise((_, reject) => {
                  timer = setTimeout(() => {
                    abortJob?.(job.id);
                    reject(new Error("execution timeout (30min)"));
                  }, EXEC_TIMEOUT);
                }),
              ]);
            } finally {
  ...
  function start() {
    ...
    console.log(`\x1b[90m[cron] 调度器已启动（间隔 ${CHECK_INTERVAL}ms）\x1b[0m`);
  }
