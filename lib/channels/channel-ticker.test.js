...
function setupFixture(agentIds = ["a", "b", "c"], channelOpts = {}) {
...
describe("channel-ticker dispatch policy", () => {
  ...
  it("auto-advances discussion channels for the configured round limit", async () => {
    const fx = setupFixture(["a", "b"], {
      mode: "discussion",
      discussionMaxRounds: 2,
    });
    tempRoots.push(fx.root);

    const calls = [];
    const executeCheck = vi.fn(async (agentId, channelName, newMessages, _allUpdates, opts = {}) => {
      calls.push({
        agentId,
        round: opts.discussionRound,
        messages: newMessages.map((m) => `${m.sender}:${m.body}`),
      });
      appendMessage(path.join(fx.channelsDir, `${channelName}.md`), agentId, `${agentId} round ${opts.discussionRound}`);
      return { replied: true, replyContent: `${agentId} round ${opts.discussionRound}` };
    });
    ...
    await ticker.triggerImmediate(fx.channelName, { source: "user" });
    await ticker.stop();

    expect(executeCheck).toHaveBeenCalledTimes(4);
    expect(calls.map((c) => `${c.round}:${c.agentId}`)).toEqual(["1:a", "1:b", "2:a", "2:b"]);
    const round2A = calls.find((c) => c.agentId === "a" && c.round === 2);
    const round2B = calls.find((c) => c.agentId === "b" && c.round === 2);
    expect(round2A.messages.some((m) => m.includes("b round 1"))).toBe(true);
    expect(round2B.messages.some((m) => m.includes("a round 1"))).toBe(true);
    expect(events.filter((e) => e.event === "channel_discussion_round_done")).toHaveLength(2);
  });
  ...
  it("routes command-mode user tasks to channel leaders before followers", async () => {
    const fx = setupFixture(["designer", "leader", "writer"], {
      mode: "command",
      leaders: ["leader"],
    });
    tempRoots.push(fx.root);
    ...
    const ticker = createChannelTicker({
      channelsDir: fx.channelsDir,
      agentsDir: fx.agentsDir,
      getAgentOrder: () => ["designer", "leader", "writer", "outsider"],
      executeCheck: vi.fn(async (agentId, _channelName, _messages, _updates, opts) => {
        calls.push({ agentId, forceReply: opts?.forceReply });
        return { replied: true };
      }),
      onMemorySummarize: vi.fn(),
    });
    ...
    expect(calls).toEqual([{ agentId: "leader", forceReply: true }]);
  });
