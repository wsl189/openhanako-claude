...
/** 消息 header 正则：### sender | YYYY-MM-DD HH:MM[:SS][.SSS]（兼容旧格式） */
const MSG_HEADER_RE = /^### (.+?) \| (\d{4}-\d{2}-\d{2} \d{2}:\d{2}(?::\d{2})?(?:\.\d{3})?)$/;
...
const CHANNEL_LEADERS_META_KEY = "leaders";
const CHANNEL_MODE_META_KEY = "mode";
const CHANNEL_DISCUSSION_MAX_ROUNDS_META_KEY = "discussion_max_rounds";
const CHANNEL_MODES = new Set(["command", "discussion"]);
let _lastGeneratedTimestampMs = 0;
 ...
 * @param {string[]} [opts.leaders] - 频道负责人列表（必须属于 members）
 * @param {"command"|"discussion"} [opts.mode] - 频道运作模式
 * @param {number} [opts.discussionMaxRounds] - 平等讨论模式的自动轮数上限
 * @param {string} [opts.intro] - 频道介绍（作为第一条系统消息）
 * @returns {{ filePath: string, id: string }}
 */
export function createChannel(channelsDir, { id, name, description, members, intro, leaders, mode, discussionMaxRounds }) {
  ...
  const meta = { id: channelId, members, [CHANNEL_MODE_META_KEY]: normalizedMode };
  if (normalizedMode === "discussion") {
    meta[CHANNEL_DISCUSSION_MAX_ROUNDS_META_KEY] = normalizeDiscussionMaxRounds(discussionMaxRounds);
  } else {
    const normalizedLeaders = normalizeStringList(leaders).filter((leaderId) => members?.includes?.(leaderId));
    if (normalizedLeaders.length > 0) meta[CHANNEL_LEADERS_META_KEY] = normalizedLeaders;
  }
...
export function appendMessage(filePath, sender, body) {
  const ts = createComparableTimestamp();
  const block = `\n### ${sender} | ${ts}\n\n${body.trim()}\n\n---\n`;
...
export function normalizeChannelMode(mode) {
  const value = String(mode || "").trim().toLowerCase();
  return CHANNEL_MODES.has(value) ? value : "command";
}

export function normalizeDiscussionMaxRounds(value) {
  const n = Math.floor(Number(value));
  if (!Number.isFinite(n)) return 3;
  return Math.min(8, Math.max(1, n));
}

export function getChannelModeFromMeta(meta) {
  return normalizeChannelMode(meta?.[CHANNEL_MODE_META_KEY]);
}

export function getDiscussionMaxRoundsFromMeta(meta) {
  return normalizeDiscussionMaxRounds(meta?.[CHANNEL_DISCUSSION_MAX_ROUNDS_META_KEY]);
}

/**
 * 从频道 meta 中读取负责人列表。
 * 兼容旧/手写字段：leaders、leader、leaderAgentId。
 * @param {object} meta
 * @returns {string[]}
 */
export function getChannelLeadersFromMeta(meta) {
  const raw = [];
  if (Array.isArray(meta?.[CHANNEL_LEADERS_META_KEY])) raw.push(...meta[CHANNEL_LEADERS_META_KEY]);
  if (Array.isArray(meta?.leader)) raw.push(...meta.leader);
  if (typeof meta?.leader === "string") raw.push(meta.leader);
  if (typeof meta?.leaderAgentId === "string") raw.push(meta.leaderAgentId);
 ...
 * @returns {string[]}
 */
export function getChannelLeaders(filePath) {
  if (!fs.existsSync(filePath)) return [];
  return getChannelLeadersFromMeta(getChannelMeta(filePath));
}

/**
 * 更新频道负责人列表（写入 frontmatter）。
 * @param {string} filePath
 * @param {string[]} leaders
 * @returns {boolean}
 */
export function setChannelLeaders(filePath, leaders = []) {
  if (!fs.existsSync(filePath)) return false;
  const next = normalizeStringList(leaders);
  return rewriteFrontmatter(filePath, (meta) => {
    const prev = getChannelLeadersFromMeta(meta);
    const same = prev.length === next.length && prev.every((v, i) => v === next[i]);
    const hasLegacyLeader = meta.leader !== undefined || meta.leaderAgentId !== undefined;
    delete meta.leader;
    delete meta.leaderAgentId;
    if (same && Array.isArray(meta[CHANNEL_LEADERS_META_KEY]) && !hasLegacyLeader) return false;
    if (next.length > 0) {
      meta[CHANNEL_LEADERS_META_KEY] = next;
    } else {
      delete meta[CHANNEL_LEADERS_META_KEY];
    }
    return true;
  });
}
...
export function normalizeChannelMembersToAgentIds(filePath, agents = []) {
  ...
  const resolveAgentToken = (raw) => {
    const token = String(raw || "").trim();
    if (!token) return "";
    const key = token.toLowerCase();
    return idMap.get(key) || nameMap.get(key) || token;
  };
  ...
  const wrote = rewriteFrontmatter(filePath, (meta) => {
    ...
    for (const raw of getChannelLeadersFromMeta(meta)) {
      const resolved = resolveAgentToken(raw);
      if (!resolved || !out.includes(resolved) || leaderSeen.has(resolved)) {
        localChanged = true;
        continue;
      }
      if (resolved !== raw) localChanged = true;
      leaderSeen.add(resolved);
      leaderOut.push(resolved);
    }
    const prevLeaders = getChannelLeadersFromMeta(meta);
    const leadersChanged = prevLeaders.length !== leaderOut.length || prevLeaders.some((v, i) => v !== leaderOut[i]);
    if (!localChanged && !leadersChanged && out.length === rawMembers.length && out.every((v, i) => v === rawMembers[i])) {
      return false;
    }
    meta.members = out;
    delete meta.leader;
    delete meta.leaderAgentId;
    if (leaderOut.length > 0) {
      meta[CHANNEL_LEADERS_META_KEY] = leaderOut;
    } else {
      delete meta[CHANNEL_LEADERS_META_KEY];
    }
    changed = true;
...
export function removeChannelMember(filePath, memberId) {
  ...
  rewriteFrontmatter(filePath, (meta) => {
    ...
    meta.members = members;
    const leaders = getChannelLeadersFromMeta(meta).filter((id) => id !== memberId && members.includes(id));
    if (leaders.length > 0) {
      meta[CHANNEL_LEADERS_META_KEY] = leaders;
    } else {
      delete meta[CHANNEL_LEADERS_META_KEY];
    }
    delete meta.leader;
...
/**
 * 生成可比较且单调递增的本地时间戳，避免同毫秒碰撞吞消息。
 * @param {Date} [date]
 * @returns {string}
 */
function createComparableTimestamp(date = new Date()) {
  let ms = date.getTime();
  if (ms <= _lastGeneratedTimestampMs) {
    ms = _lastGeneratedTimestampMs + 1;
  }
  _lastGeneratedTimestampMs = ms;
