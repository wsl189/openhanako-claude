...
export function createChannelTicker({
   ...
   */
  async function runMentionDispatch(channelName, allAgents, mentionedAgents, signal) {
  ...
  async function runDispatch(channelName, { mentionedAgents, source = "user", signal } = {}) {
    ...
    if (mentions.length > 0) {
      await runMentionDispatch(channelName, allAgents, mentions, signal);
      return;
    }
  ...
  function enqueueDispatch(task, { version = 0 } = {}) {
    const run = async () => {
      if (_stopped) return;
      if (version < _latestUserInputVersion) {
        debugLog()?.log("ticker", `drop stale dispatch task (v${version} < v${_latestUserInputVersion})`);
        return;
      }
      _runningJobs += 1;
  ...
  function abortRunningDispatch(reason = "new-user-input") {
  ...
  function triggerImmediate(channelName, { mentionedAgents, source = "user" } = {}) {
    ...
    if (normalizedSource === "user") {
      _latestUserInputVersion += 1;
      const aborted = abortRunningDispatch("new-user-input");
      if (aborted) {
        debugLog()?.log("ticker", `user input interrupted running replies (#${channelName})`);
      }
    }
    const version = _latestUserInputVersion;
