 ...
 * - 调度按频道排队、按 agent 串行：不同频道/不同 agent 可并行，同频道和同 agent 保持顺序
 */
...
export function createChannelTicker({
  ...
  let _stopped = true;
  const _dispatchChainByChannel = new Map();
  const _agentChainByAgent = new Map();
  const _memoryChainByAgent = new Map();
  let _runningJobs = 0;
  const _activeDispatchControllers = new Set();
  ...
  function enqueueMapTask(chains, key, task) {
  ...
  function getLatestTimestamp(channelFile) {
    ...
    const headerRe = /^### .+? \| (\d{4}-\d{2}-\d{2} \d{2}:\d{2}(?::\d{2})?(?:\.\d{3})?)$/gm;
    let lastMatch = null;
    let m;
  ...
  /**
   * 将本次成功处理过的 bookmark 对齐到指定时间戳。
   * 用于 mention 并行波次：保留“波次内新消息”为未读，以支持级联 @ 激活。
   *
   * @param {string} channelName
   * @param {Array<{ channelsMdPath: string }>} processedEntries
   * @param {string|null} timestamp
   */
  function syncProcessedBookmarksToTimestamp(channelName, processedEntries, timestamp) {
    if (!processedEntries?.length || !timestamp) return;
    const dedup = new Set(processedEntries.map(e => e.channelsMdPath).filter(Boolean));
    for (const channelsMdPath of dedup) {
      const bookmarks = readBookmarks(channelsMdPath);
      if (!bookmarks.has(channelName)) continue;
   ...
   */
  async function processAgentOnce(agentId, channelName, opts = {}) {
    const { forceReply = false, signal } = opts;
    ...
    try {
      const result = await executeCheck(agentId, channelName, unread, [], { ...opts, forceReply, signal });
      if (signal?.aborted) return { status: "aborted" };
      ...
      return { status: "ok", channelsMdPath, replied: !!result?.replied };
    } catch (err) {
  ...
  function enqueueAgentProcess(agentId, channelName, opts = {}) {
    if (!agentId) return Promise.resolve({ status: "skip" });
    return enqueueMapTask(
      _agentChainByAgent,
      agentId,
      () => processAgentOnce(agentId, channelName, opts),
    );
  }

  function getChannelDispatchConfig(channelName) {
    const channelFile = path.join(channelsDir, `${channelName}.md`);
    ...
    return {
      ...
      members: Array.isArray(meta.members) ? meta.members.map((id) => String(id || "").trim()).filter(Boolean) : [],
      leaders: getChannelLeadersFromMeta(meta),
    };
  }

  function orderKnownAgents(preferredOrder, candidates) {
    const wanted = new Set((candidates || []).map((id) => String(id || "").trim()).filter(Boolean));
    if (wanted.size === 0) return [];
    const out = [];
    const seen = new Set();
    for (const id of preferredOrder || []) {
      const agentId = String(id || "").trim();
      if (!wanted.has(agentId) || seen.has(agentId)) continue;
      seen.add(agentId);
      out.push(agentId);
    }
    for (const id of wanted) {
      if (seen.has(id)) continue;
      if (fs.existsSync(path.join(agentsDir, id))) out.push(id);
    }
    return out;
  }

  function leadersFirst(agents, leaders) {
    const leaderSet = new Set((leaders || []).map((id) => String(id || "").trim()).filter(Boolean));
    if (leaderSet.size === 0) return agents;
    return [
      ...agents.filter((id) => leaderSet.has(id)),
      ...agents.filter((id) => !leaderSet.has(id)),
    ];
  }
  ...
  async function runMentionDispatch(channelName, allAgents, mentionedAgents, signal, { source = "user" } = {}) {
    ...
    const settled = await Promise.allSettled(
      agents.map(agentId => enqueueAgentProcess(agentId, channelName, { forceReply: true, signal })),
    );
    if (signal?.aborted) return;
    ...
    // mention 并行波次：参与者书签回对齐到“波次开始时间”。
    // 这样波次内新产生的 @ 消息仍是未读，下一跳可继续被激活。
    syncProcessedBookmarksToTimestamp(channelName, processedEntries, dispatchStartLatestTs);
  ...
  async function runUserTriageDispatch(channelName, allAgents, signal) {
    ...
    for (const agentId of allAgents) {
      ...
      const r = await enqueueAgentProcess(agentId, channelName, { forceReply: false, signal });
      if (r?.status === "ok" && r.channelsMdPath) {
        processedEntries.push({ channelsMdPath: r.channelsMdPath });
      }
    }
  ...
  async function runDiscussionDispatch(channelName, allAgents, mentionedAgents, signal, { maxRounds = 3 } = {}) {
    if (signal?.aborted) return;
    const channelFile = path.join(channelsDir, `${channelName}.md`);
    const participants = [];
    const seen = new Set();
    const requested = Array.isArray(mentionedAgents) && mentionedAgents.length > 0
      ? mentionedAgents
      : allAgents;
    const cached = new Set(allAgents || []);
    for (const id of requested || []) {
      const agentId = String(id || "").trim();
      if (!agentId || seen.has(agentId)) continue;
      if (!cached.has(agentId) && !fs.existsSync(path.join(agentsDir, agentId))) continue;
      seen.add(agentId);
      participants.push(agentId);
    }
    if (participants.length === 0) return;

    const rounds = Math.min(8, Math.max(1, Number(maxRounds) || 3));
    console.log(`\x1b[90m[channel-ticker] #${channelName} 平等讨论（最多 ${rounds} 轮）：${participants.join(", ")}\x1b[0m`);
    debugLog()?.log("ticker", `discussion dispatch #${channelName}: rounds=${rounds}, agents=${participants.join(",")}`);
    onEvent?.("channel_dispatch_start", { channelName, mode: "discussion", agents: participants, maxRounds: rounds });

    const allProcessedEntries = [];
    for (let round = 1; round <= rounds; round += 1) {
      if (signal?.aborted) break;
      const roundStartLatestTs = getLatestTimestamp(channelFile);
      onEvent?.("channel_discussion_round_start", { channelName, round, maxRounds: rounds, agents: participants });

      const settled = await Promise.allSettled(
        participants.map(agentId => enqueueAgentProcess(agentId, channelName, {
          forceReply: true,
          signal,
          discussionMode: true,
          discussionRound: round,
          discussionMaxRounds: rounds,
        })),
      );
      if (signal?.aborted) return;

      const roundProcessedEntries = [];
      let repliedCount = 0;
      for (const item of settled) {
        const r = item.status === "fulfilled" ? item.value : null;
        if (r?.status === "ok" && r.channelsMdPath) {
          roundProcessedEntries.push({ channelsMdPath: r.channelsMdPath });
          allProcessedEntries.push({ channelsMdPath: r.channelsMdPath });
        }
        if (r?.replied) repliedCount += 1;
      }

      onEvent?.("channel_discussion_round_done", { channelName, round, maxRounds: rounds, agents: participants, repliedCount });
      if (round < rounds && repliedCount > 0) {
        syncProcessedBookmarksToTimestamp(channelName, roundProcessedEntries, roundStartLatestTs);
        continue;
      }
      break;
    }
    ...
    syncProcessedBookmarksToLatest(channelFile, channelName, allProcessedEntries);
    onEvent?.("channel_dispatch_done", { channelName, mode: "discussion", agents: participants });
  }
  ...
  async function runDispatch(channelName, { mentionedAgents, source = "user", signal } = {}) {
    ...
    const dispatchConfig = getChannelDispatchConfig(channelName);
    const channelAgents = orderKnownAgents(allAgents, dispatchConfig.members);
    const commandAgents = leadersFirst(channelAgents, dispatchConfig.leaders);

    const mentions = Array.isArray(mentionedAgents)
      ? [...new Set(mentionedAgents.filter((id) => channelAgents.includes(id)))]
      : [];

    if (source === "user" && dispatchConfig.mode === "discussion") {
      await runDiscussionDispatch(channelName, channelAgents, mentions, signal, {
        maxRounds: dispatchConfig.discussionMaxRounds,
      });
      return;
    }

    if (mentions.length > 0) {
      await runMentionDispatch(channelName, channelAgents, mentions, signal, { source });
      return;
    }
    ...
    // 只允许用户“无 @”触发顺序 triage
    if (source === "user") {
      if (dispatchConfig.mode === "command" && dispatchConfig.leaders.length > 0) {
        await runMentionDispatch(channelName, commandAgents, dispatchConfig.leaders, signal, { source });
        return;
      }
      await runUserTriageDispatch(channelName, commandAgents, signal);
      return;
    }
  ...
  function enqueueDispatch(channelName, task, { version = 0 } = {}) {
    const run = async () => {
      if (_stopped) return;
      ...
      _activeDispatchControllers.add(controller);
      try {
        ...
        _activeDispatchControllers.delete(controller);
        _runningJobs = Math.max(0, _runningJobs - 1);
      }
    };
    return enqueueMapTask(_dispatchChainByChannel, channelName, run);
  }

  function abortRunningDispatch(reason = "manual-stop") {
    let aborted = false;
    for (const ctrl of _activeDispatchControllers) {
      if (!ctrl || ctrl.signal.aborted) continue;
  ...
  async function stop() {
    ...
    await Promise.allSettled([
      ..._dispatchChainByChannel.values(),
      ..._agentChainByAgent.values(),
    ]);
  }
