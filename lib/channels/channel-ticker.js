 ...
 * @param {boolean} [opts.enablePolling=true] - 是否启用周期轮询
...
export function createChannelTicker({
  ...
  /**
   * 将本轮成功处理过的 bookmark 对齐到频道最终最新时间戳。
   * 这样可避免“先处理的 agent 落后于后处理 agent”导致下次重启/轮询重复触发。
   *
  ...
  /**
   * 当用户消息显式 @ 了部分 agent 时，将未被 @ 的 agent 书签推进到最新，
   * 避免后续周期轮询把同一条消息再次当作“新消息”处理。
   *
   * @param {string} channelFile
   * @param {string} channelName
   ...
   */
  function markUnmentionedAgentsAsRead(channelFile, channelName, allAgents, mentionedAgents) {
    const latestTs = getLatestTimestamp(channelFile);
    if (!latestTs) return;
  ...
  async function _runCycle() {
    ...
    try {
      ...
      const processedByChannel = new Map(); // channelName -> { channelFile, entries[] }
      ...
      for (let ai = startAgent; ai < agents.length; ai++) {
        ...
        for (let ci = startCh; ci < channelUpdates.length; ci++) {
          ...
          const processResult = await _processOneChannel(agentId, update);
          if (processResult?.status === "ok") {
            let record = processedByChannel.get(update.channelName);
            if (!record) {
              record = { channelFile: update.channelFile, entries: [] };
              processedByChannel.set(update.channelName, record);
            }
            record.entries.push({ channelsMdPath: update.channelsMdPath });
          }
        }
      }

      // 对齐每个频道本轮成功处理者的 bookmark 到最终最新消息，防止顺序漂移。
      for (const [channelName, record] of processedByChannel) {
        syncProcessedBookmarksToLatest(record.channelFile, channelName, record.entries);
      }
   ...
   *
   * @param {string} channelName
   * @param {{ mentionedAgents?: string[], interruptCurrent?: boolean }} [opts]
   */
  function triggerImmediate(channelName, { mentionedAgents, interruptCurrent = true } = {}) {
    if (_stopped) return Promise.resolve();

    // 抢占模式：立刻打断当前 triage（用户消息默认如此）。
    // 非抢占模式：仅排队，等当前 triage 自然结束（用于 agent -> agent @ 触发）。
    if (interruptCurrent && _triageAbortCtrl) {
      console.log(`\x1b[90m[channel-ticker] 新消息到达，立即 abort 当前 triage\x1b[0m`);
      debugLog()?.log("ticker", `new message arrived, abort current triage now`);
      _triageAbortCtrl.abort();
    }

    // 串行化：等待上一次 triage 清理完成后再启动下一次，避免并发重入
  ...
  async function _doTriage(channelName, { mentionedAgents } = {}) {
    ...
    // ── 4. triage 执行 ──
    const processImmediateAgent = async (agentId) => {
      ...
      // 防重放：只有该 agent 在此频道确实存在“未读新消息”时才执行 triage。
      // 避免同一条 @ 消息在重复 triggerImmediate 时被反复处理。
      const bookmark = bookmarks.get(channelName);
      const bk = bookmark === "never" ? null : bookmark;
      const unread = getNewMessages(channelFile, bk, agentId);
      if (unread.length === 0) return { status: "skip" };

      const recentMsgs = getRecentMessages(channelFile, 20, agentId);
      if (recentMsgs.length === 0) return { status: "skip" };

      console.log(`\x1b[90m[channel-ticker] 立即 ${agentId} → #${channelName}（${recentMsgs.length} 条上下文）\x1b[0m`);

      try {
        const forceReply = hasMentions && mentionedAgents.includes(agentId);
        const result = await executeCheck(agentId, channelName, recentMsgs, [], { signal, forceReply });
        if (signal.aborted) return { status: "aborted" };
    ...
    try {
      const runInParallel = hasMentions && agents.length > 1;

      if (runInParallel) {
        const settled = await Promise.allSettled(agents.map(agentId => processImmediateAgent(agentId)));
        for (const item of settled) {
          const r = item.status === "fulfilled" ? item.value : { status: "error" };
          if (r?.status === "ok" && r.channelsMdPath) {
            processedEntries.push({ channelsMdPath: r.channelsMdPath });
          }
        }
      } else {
        // 顺序模式：每次重新读滑动窗口，让后续 agent 能看到前序回复。
        for (const agentId of agents) {
          ...
          const r = await processImmediateAgent(agentId);
          if (r?.status === "ok" && r.channelsMdPath) {
            processedEntries.push({ channelsMdPath: r.channelsMdPath });
          }
        }
      ...
      if (!signal.aborted) {
        // triage 全部完成后再统一对齐，避免前序 agent 的 bookmark 落后于后序回复。
        syncProcessedBookmarksToLatest(channelFile, channelName, processedEntries);
        if (hasMentions) {
          // 定向 @ 场景：未被 @ 的 agent 视为“旁观已读”，不再触发后续全员补回复。
          markUnmentionedAgentsAsRead(channelFile, channelName, allAgents, mentionedAgents);
        }
  ...
  function start() {
    ...
    if (!enablePolling) {
      console.log(`\x1b[90m[channel-ticker] 调度器已启动（仅事件驱动，周期轮询已关闭）\x1b[0m`);
      debugLog()?.log("ticker", "started in event-driven mode, polling disabled");
      return;
    }
