...
function inferAttachmentType(att = {}) {
  ...
  if (att.voice_wav_url || att.asr_refer_text) return "audio";

  const ext = extractExtFromPathLike(att.filename || att.url || att.voice_wav_url);
...
function inferAttachmentMime(att = {}) {
  ...
  if (contentType === "voice") {
    if (att.voice_wav_url) return "audio/wav";
    return MIME_BY_EXT[ext] || "audio/amr";
  }
  if (att.voice_wav_url) return "audio/wav";
  if (att.asr_refer_text) return MIME_BY_EXT[ext] || "audio/amr";
  return MIME_BY_EXT[ext] || contentType;
}

function inferTypeFromHintText(text = "") {
  const s = String(text || "").toLowerCase();
  if (!s) return "file";
  if (/audio|voice|语音|录音|amr|silk/.test(s)) return "audio";
  if (/image|photo|图片|照片/.test(s)) return "image";
  if (/video|视频/.test(s)) return "video";
  return "file";
}

function normalizeMediaAttachment(media = {}, data = {}) {
  const fileType = Number(media.file_type || media.type || data.file_type || 0);
  const filename = String(media.filename || media.file_name || "").trim();
  const contentType = String(media.content_type || "").trim().toLowerCase();
  const url = String(media.url || media.file_url || media.voice_wav_url || data.voice_wav_url || "").trim();
  const voiceWavUrl = String(media.voice_wav_url || data.voice_wav_url || "").trim();
  const asrReferText = String(media.asr_refer_text || data.asr_refer_text || "").trim();
  const ext = extractExtFromPathLike(filename || url || voiceWavUrl);
  const hintedType = inferTypeFromHintText(
    `${media.content_type || ""} ${filename} ${data.content || ""}`,
  );
  let type = "file";
  if (fileType === 1) type = "image";
  else if (fileType === 2) type = "video";
  else if (fileType === 3) type = "audio";
  else if (voiceWavUrl || asrReferText) type = "audio";
  else if (hintedType !== "file") type = hintedType;
  ...
  return {
    type,
    url: url || undefined,
    filename: filename || (type === "audio" ? "voice.amr" : "media.bin"),
    mimeType: String(
      (contentType.startsWith("image/") || contentType.startsWith("video/") || contentType.startsWith("audio/")
        ? contentType
        : "")
      || MIME_BY_EXT[ext]
      || (voiceWavUrl ? "audio/wav" : "")
      || (type === "audio" ? "audio/amr" : ""),
    ).trim().toLowerCase(),
    size: media.size || media.file_size,
    duration: media.duration || data.duration,
    voiceWavUrl: voiceWavUrl || undefined,
...
/** 从 QQ v2 API 事件的 data.attachments/media 提取统一附件 */
export function normalizeQQAttachments(data = {}) {
  const attachments = [];
  if (data.attachments?.length) {
    for (const att of data.attachments) {
      ...
      attachments.push({
        type,
        url: att.url || att.voice_wav_url,
        filename: att.filename,
        mimeType: inferAttachmentMime(att), size: att.size,
        width: att.width, height: att.height,
        duration: att.duration,
        voiceWavUrl: att.voice_wav_url,
        asrReferText: att.asr_refer_text,
        platformRef: att.file_info || att.file_uuid || att.file_id || att.id,
      });
    }
  }

  if (!attachments.length && data?.media && typeof data.media === "object") {
    attachments.push(normalizeMediaAttachment(data.media, data));
  }

  if (!attachments.length) {
    const msgType = Number(data?.msg_type || data?.message_type || 0);
    const hinted = inferTypeFromHintText(data?.content || "");
    if (msgType === 7 || hinted !== "file") {
      attachments.push({
        type: hinted === "file" ? "audio" : hinted,
        filename: hinted === "audio" ? "voice.amr" : "media.bin",
        mimeType: hinted === "audio" ? "audio/amr" : "",
      });
    }
  }
  return attachments;
...
export function createQQAdapter({ appID, appSecret, onMessage, dmGuildMap, onDmGuildDiscovered, onStatus }) {
  ...
  function extractAttachments(data) {
    return normalizeQQAttachments(data);
  }
  ...
  function handleEvent(type, data) {
    ...
    const isC2C = eventType === "C2C_MESSAGE_CREATE"
      || /^C2C_.*MESSAGE_CREATE$/i.test(eventType)
      || /^C2C_MESSAGE_/i.test(eventType);
    const isGroupAt = eventType === "GROUP_AT_MESSAGE_CREATE"
      || /^GROUP_.*MESSAGE_CREATE$/i.test(eventType);
    const isAtMessage = eventType === "AT_MESSAGE_CREATE";
    const isDirectMessage = eventType === "DIRECT_MESSAGE_CREATE"
      || /^DIRECT_.*MESSAGE_CREATE$/i.test(eventType);
    ...
    if (!handled && /MESSAGE/i.test(eventType)) {
      const msgType = Number(data?.msg_type || data?.message_type || 0);
      const contentLen = String(data?.content || "").trim().length;
      const attachmentsCount = Array.isArray(data?.attachments) ? data.attachments.length : 0;
      const hasMedia = Boolean(data?.media);
      debugLog()?.log(
        "bridge",
        `[qq] 未处理消息事件 type=${eventType} msg_type=${msgType} contentLen=${contentLen} attachments=${attachmentsCount} media=${hasMedia ? "1" : "0"}`,
      );
    }
  }
  ...
  return {
    ...
    async downloadAudio(platformRef) {
      const ref = String(platformRef || "").trim();
      if (!ref) throw new Error("empty audio platformRef");
      if (/^https?:\/\//i.test(ref)) {
        return this.downloadAttachment(ref);
      }
      throw new Error("unsupported audio platformRef");
    },
