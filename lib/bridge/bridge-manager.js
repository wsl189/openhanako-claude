...
export class BridgeManager {
  ...
  autoStart() {
    ...
    // 单 bot 平台
    for (const platform of ["feishu"]) {
      this.startPlatformFromConfig(platform, bridge[platform] || {});
    }

    // 多 bot 平台
    for (const platform of ["telegram", "qq"]) {
      this.startPlatformFromConfig(platform, bridge[platform] || {});
    }
  }
  ...
  async _handleMessage(platformKey, msg) {
    ...
    if (/^\/(stop|abort)$/i.test(text.trim())) {
  ...
  async _flushPending(sessionKey) {
    ...
    // QQ 在分块流式回发时容易出现后续分块丢失，导致平台侧只看到第一段。
    // 这里对 QQ 强制走 batch，优先保证整条消息完整送达。
    const canStream = !!adapter?.sendBlockReply && !isGroup && platform !== "qq";
