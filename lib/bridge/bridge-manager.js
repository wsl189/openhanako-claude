  ...
  _processLine(line) {
    ...
    if (this._inMediaTag) {
      ...
      if (closeIdx !== -1) {
        ...
        const source = this._mediaTagLines.join("\n").trim();
        this._mediaTagLines = [];
        this._inMediaTag = false;
        if (source) this.extractedMedia.push(source);
      } else {
    ...
    if (/^<media>/i.test(trimmed)) {
      ...
      if (closeIdx !== -1) {
        const source = afterOpen.slice(0, closeIdx).trim();
        if (source) this.extractedMedia.push(source);
      } else {
...
export class BridgeManager {
  ...
  _hasExplicitMediaSendIntent(userPrompt = "") {
    ...
    // 明确否定优先：不要发送/不用发
    if (/(不要|别|无需|不用|不需要|先别|暂时别).{0,8}(发|发送|传|上传|分享)/.test(text)) return false;
    if (/(don't|do not|no need to|not need to).{0,20}(send|upload|attach|share)/i.test(lower)) return false;

    // 中英文显式“发送媒体”意图
    if (/(发给我|发我|传给我|传我|给我发|给我传|发送给我|上传给我)/.test(text)) return true;
    if (/(把.{0,24}(图片|图|照片|截图|文件|文档|视频|音频|语音|pdf).{0,10}(发|传|上传))/.test(text)) return true;
    if (/(send (me|it|them)|send .* to me|upload .*?(image|file|photo|screenshot|video|audio|document)|attach .*?(image|file|photo|document)|share .*?(image|file|photo|document))/i.test(lower)) return true;
  ...
  _shouldDispatchMedia(userPrompt = "", nonMediaReplyText = "") {
    // 规则：
    // 1) 用户明确要求发送媒体 => 允许
    // 2) 回复只有媒体标记、没有普通文本 => 允许（兼容旧流程）
    if (this._hasExplicitMediaSendIntent(userPrompt)) return true;
    return !String(nonMediaReplyText || "").trim();
  }

  _buildMediaSuppressedText(userPrompt = "") {
    const isZh = /[\u4e00-\u9fff]/.test(String(userPrompt || ""));
    if (isZh) {
      return "检测到媒体路径，但你没有明确要求发送。我先不自动发送；需要时请直接说“把图片/文件发给我”。";
    }
    return "I detected media paths, but you didn't explicitly ask me to send them. I won't auto-send; ask me to send the image/file when needed.";
  }
