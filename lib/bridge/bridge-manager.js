...
async function transcribeAudioWithEngine({
  ...
  for (const endpoint of endpointCandidates) {
    for (const item of modelCandidates) {
      const uploadExt = normalizeAudioExt(fileName) || inferAudioExtFromMime(mimeType) || "webm";
  ...
  throw new Error(`${lastError}${endpointSuffix}`);
}
    ...
    for (const att of attachments) {
      try {
        if (att.type === "image") {
          ...
          const fallbackMime = inferAudioMimeFromPath(att.filename || att.url);
          try {
            if (att.url && adapter?.downloadAttachment) {
              audioBuffer = await adapter.downloadAttachment(att.url);
            } else if (att.url) {
              audioBuffer = await downloadMedia(att.url);
          ...
          if (audioBuffer?.length > 0 && audioBuffer.length <= BRIDGE_TRANSCRIBE_MAX_AUDIO_BYTES) {
        const mimeType = detectMime(
          audioBuffer,
          att.mimeType || fallbackMime || "audio/webm",
        );
        debugLog()?.log(
          "bridge",
          `voice attachment meta file=${String(att.filename || "").slice(-48) || "(none)"} `
          + `mime=${mimeType} size=${audioBuffer.length}`,
        );
        try {
