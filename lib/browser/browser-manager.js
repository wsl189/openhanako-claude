...
export class BrowserManager {
  constructor() {
    ...
    if (process.send) {
      process.on("message", (msg) => {
        ...
        if (msg?.type === "browser-state-sync") {
          this._running = !!msg.running;
          this._url = msg.url || null;
        }
      });
    }
  }
  ...
  /** 是否为主进程返回的“浏览器未启动”错误 */
  _isNotLaunchedError(err) {
    const msg = String(err?.message || "");
    return msg.includes("Browser not launched") || msg.includes("Call start first");
  }

  /**
   * 确保浏览器可用：优先恢复当前 session 的浏览器状态（热/冷恢复）
   * 说明：某些流程会把浏览器挂起（_running=false），这里自动恢复，避免后续动作误报“未启动”
   */
  ...
  /**
   * 发送浏览器命令，遇到“未启动”状态时自动尝试恢复并重试一次
   * @param {string} cmd
   * @param {object} params
   * @returns {Promise<any>}
   */
  async _sendCmdWithRecovery(cmd, params = {}) {
    try {
      return await this._sendCmd(cmd, params);
    } catch (err) {
      if (!this._isNotLaunchedError(err)) throw err;
      // server 与主进程状态可能短暂失步：先标记为未运行，再尝试恢复一次
      this._running = false;
      await this._ensureRunning();
      if (!this._running) throw err;
      return this._sendCmd(cmd, params);
    }
  }
  ...
  async navigate(url) {
    await this._ensureRunning();
    const result = await this._sendCmdWithRecovery("navigate", { url });
  ...
  async snapshot() {
    await this._ensureRunning();
    const result = await this._sendCmdWithRecovery("snapshot");
  ...
  async screenshot() {
    await this._ensureRunning();
    const result = await this._sendCmdWithRecovery("screenshot");
  ...
  async thumbnail() {
    try {
      await this._ensureRunning();
      const result = await this._sendCmdWithRecovery("thumbnail");
  ...
  async click(ref) {
    await this._ensureRunning();
    const result = await this._sendCmdWithRecovery("click", { ref });
    this._url = result.currentUrl;
    return result.text;
  }
  ...
  async type(text, ref, { pressEnter = false } = {}) {
    await this._ensureRunning();
    const result = await this._sendCmdWithRecovery("type", { text, ref, pressEnter });
    this._url = result.currentUrl;
    return result.text;
  }
  ...
  async scroll(direction, amount = 3) {
    await this._ensureRunning();
    const result = await this._sendCmdWithRecovery("scroll", { direction, amount });
    return result.text;
  }
  ...
  async select(ref, value) {
    await this._ensureRunning();
    const result = await this._sendCmdWithRecovery("select", { ref, value });
    return result.text;
  }
  ...
  async pressKey(key) {
    await this._ensureRunning();
    const result = await this._sendCmdWithRecovery("pressKey", { key });
    return result.text;
  }
  ...
  async wait(opts = {}) {
    await this._ensureRunning();
    const result = await this._sendCmdWithRecovery("wait", opts);
    return result.text;
  }
  ...
  async evaluate(expression) {
    await this._ensureRunning();
    const result = await this._sendCmdWithRecovery("evaluate", { expression });
    return result.value;
  }
  ...
  async show() {
    await this._ensureRunning();
    await this._sendCmdWithRecovery("show");
  }
}
