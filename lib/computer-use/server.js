...
async function listDisplays() {
  ...
  const raw = await runJxa(`
    ...
    for (var i = 0; i < count; i++) {
      ...
      result.push({
        displayId: Number(did),
        width: Number(w),
        height: Number(h),
        name: "Display " + (i + 1) + " (#" + Number(did) + ")"
      });
    }
    JSON.stringify(result);
  `);
...
async function runAction(toolName, args, state) {
  ...
  if (toolName === "screenshot") {
    ...
    const shot = await captureScreenshot({
      displayId: Number.isFinite(state.selectedDisplayId) ? Number(state.selectedDisplayId) : undefined,
    ...
    const displays = await listDisplays();
    const selected = Number.isFinite(state.selectedDisplayId)
      ? displays.find((d) => Number(d.displayId) === Number(state.selectedDisplayId))
      : null;
    const note = selected
  ...
  if (toolName === "cursor_position") {
    ...
    return makeTextResult(`Cursor position: (${pos.x}, ${pos.y})`, pos);
  }
  ...
  if (toolName === "mouse_move") {
    await ensureFrontmostAllowed(state);
    const { x, y } = normalizeCoord(params.coordinate);
    await moveMouseTo(x, y);
    return makeTextResult(`Moved mouse to (${x}, ${y}).`);
  }
  ...
  if (toolName === "left_click" || toolName === "double_click" || toolName === "triple_click"
    ...
    return makeTextResult(`${toolName} at (${x}, ${y}).`);
  }
  ...
  if (toolName === "left_click_drag") {
    await ensureFrontmostAllowed(state);
    const end = normalizeCoord(params.coordinate);
    const start = params.start_coordinate ? normalizeCoord(params.start_coordinate) : null;
    await dragMouse({ start, end });
    state.leftMouseDown = false;
    return makeTextResult(`Dragged mouse to (${end.x}, ${end.y}).`);
  }
  ...
  if (toolName === "scroll") {
    ...
    return makeTextResult(`Scrolled ${direction} by ${Math.round(amount)} at (${x}, ${y}).`);
  }
...
export function createComputerUseMcpServer(name = "computer_use", opts = {}) {
  const coordinateDesc = "Horizontal/vertical position in image pixels from the latest screenshot.";
