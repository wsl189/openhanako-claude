...
function scanCompatibilityProjection(agentDir, hanakoHome) {
  ...
  try {
    ...
    const hasChannelMemory = Boolean(localDb.prepare(`
      SELECT EXISTS(
        SELECT 1
        FROM facts
        WHERE is_active = 1 AND scope = 'channel'
        UNION
        SELECT 1
        FROM playbooks
        WHERE active = 1 AND scope = 'channel'
        UNION
        SELECT 1
        FROM episodes
        WHERE scope = 'channel'
    ...
    `).get()?.present);
    const channelSummaryProjection = localDb.prepare(`
      SELECT content, source_scope
      FROM memory_projections
      WHERE key = ?
    `).get(CHANNEL_SUMMARY_PROJECTION_KEY);
    if (hasChannelMemory && !normalizeText(channelSummaryProjection?.content || "")) {
      issues.push({
        type: "missing_channel_summary_projection",
        fact: "current_summary_channel",
        action: "reproject",
      });
    } else if (channelSummaryProjection && normalizeText(channelSummaryProjection.source_scope) !== "channel") {
      issues.push({
