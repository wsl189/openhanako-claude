...
const SUMMARY_REASONING_PATTERNS = [
  /\b(?:let me analyze|i need to|we need to|the user is asking|the user wants|okay, the user|now let me)\b/i,
  /\b(?:existing summary|new conversation|source conversation|invalid summary|previous summary)\b/i,
  /<\/?(?:think|thinking|analysis|reasoning)\b/i,
  /(?:分析用户提供|让我分析|我需要分析|首先分析|先分析|分析如下)/,
  /用户(?:要求|希望|想让我|让我)(?:我)?(?:生成|更新|整合|总结|提取|分析)/,
  /(?:已有摘要|新增对话|原始对话|原始坏摘要|上一版摘要)/,
];
...
export function validateSummary(text, opts = {}) {
  ...
  if (SUMMARY_REASONING_PATTERNS.some((pattern) => pattern.test(summary))) {
    reasons.push("reasoning_leak");
  }
