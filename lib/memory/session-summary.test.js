...
describe("summary validation", () => {
  ...
  it("rejects model reasoning leaked inside otherwise well-formed summaries", () => {
    const text = [
      "## 重要事实",
      "用户要求我根据已有摘要和新增对话生成一份结构化摘要。",
      "让我分析新增对话的关键内容。",
      "",
      "## 事情经过",
      "01:00 用户打招呼，助手回应。",
    ...
    const result = validateSummary(text, { isZh: true });
    expect(result.ok).toBe(false);
    expect(result.reasons).toContain("reasoning_leak");
  });

  it("repairs reasoning-leaked summaries before saving", async () => {
    const manager = createManager();
    manager._callRollingLLM = async () => [
      "## 重要事实",
      "The user wants me to summarize this conversation. Let me analyze the dialogue.",
      "",
      "## 事情经过",
      "01:00 用户打招呼。",
    ].join("\n");
    ...
    expect(summary).not.toContain("Let me analyze");
