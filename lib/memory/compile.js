...
/**
 * 编译 facts.md：
 * - 优先从 facts.db（FactStore）读取“当前有效事实”，并显式标注时间
 * - 若未提供 FactStore，则回退到从近期 session 摘要提取
 * @param {object} resolvedModel
 */
export async function compileFacts(summaryManager, outputPath, resolvedModel, opts = {}) {
  ...
  // 优先路径：结构化事实库（带 is_active/valid_to 过滤）
  if (factStore && typeof factStore.getAll === "function") {
    const nowIso = new Date().toISOString();
    const rows = factStore.getAll()
      .filter((f) => {
        if (!f?.is_active) return false;
        if (f.valid_from && f.valid_from > nowIso) return false;
        if (f.valid_to && f.valid_to < nowIso) return false;
        return true;
      })
      .sort((a, b) => {
        const ta = (a.time || a.valid_from || a.created_at || "");
        const tb = (b.time || b.valid_from || b.created_at || "");
    ...
    for (const row of rows) {
      ...
      seen.add(key);
      const timeLabel = _formatFactTime(row.time || row.valid_from || row.created_at, isZh);
  ...
  for (const s of sessions) {
    // 频道摘要是群聊话题记忆，不作为“用户稳定属性”来源写入 facts.md。
    if (String(s.session_id || "").startsWith("channel-")) continue;
  ...
  const result = await _compactLLM(
    combined,
    isZh
      ? "将以下重要事实去重合并（200字以内）。只保留稳定、跨时间有效的信息。对带时效的“当前状态类”事实执行通用规则：如果无法确认仍有效则删除；同主题冲突时以最新为准。每条事实前必须加上时间标签 [YYYY-MM-DD HH:MM]，无法确定时用 [时间未知]。直接输出事实列表。"
      : "Deduplicate and merge the following key facts (under 120 words). Keep only stable time-persistent information. Apply a generic timeliness rule for current-state facts: drop those not verifiably still valid, and when conflicting keep the latest one. Prefix each fact with [YYYY-MM-DD HH:MM], or [time-unknown] if unavailable. Output the fact list directly.",
    resolvedModel,
    300,
  );
