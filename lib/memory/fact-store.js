...
export class FactStore {
  ...
  /**
   * 批量导入
   * @param {Array<{ fact, tags, time?, session_id?, timeliness?, state_key?, ttl_days?, valid_from?, valid_to?, scope?, origin?, source_refs?, truth_time?, confidence?, importance?, subject_id?, hash?, invalidated_by? }>} entries
   */
  importAll(entries) {
    const run = this.db.transaction(() => {
      for (const entry of entries) {
