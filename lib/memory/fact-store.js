...
export class FactStore {
  ...
  _ensureFactWritePathHealthy({ repaired = false } = {}) {
    ...
    try {
      ...
      if (!repaired && isMalformedSqliteError(message)) {
  ...
  _initSchema() {
    ...
    this.db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_evidence_archive_after ON evidence(archive_after, created_at DESC);
      CREATE INDEX IF NOT EXISTS idx_evidence_purge_after ON evidence(purge_after, created_at DESC);
  ...
  _migrate() {
    ...
    this.db.transaction(() => {
      ...
      this._resumeLegacyFactsBackfillIfNeeded();
      this._ensureEvidenceColumns();
  ...
  _countRows(tableName) {
    return Number(this.db.prepare(`SELECT COUNT(*) AS cnt FROM ${tableName}`).get()?.cnt || 0);
  }

  _migrateFactsTable() {
    const hasFacts = this._tableExists("facts");
    const hasLegacy = this._tableExists("facts_legacy_v2");
    ...
    if (hasFacts && (this._hasUnifiedFactsSchema() || this._hasModernFactsSchema())) {
      return;
    }
  ...
  _resumeLegacyFactsBackfillIfNeeded() {
    if (!this._tableExists("facts") || !this._tableExists("facts_legacy_v2")) return;
    const missingBefore = this.db.prepare(`
      SELECT COUNT(*) AS cnt
      FROM facts_legacy_v2 legacy
      WHERE NOT EXISTS (
        SELECT 1
        FROM facts
        WHERE facts.id = legacy.id
      )
    `).get()?.cnt || 0;
    if (!missingBefore) return;

    const columns = this.db.prepare(`PRAGMA table_info(facts_legacy_v2)`).all().map((row) => row.name);
    const has = (name) => columns.includes(name);
    const legacyMemoryKind = has("memory_kind") ? "NULLIF(TRIM(legacy.memory_kind), '')" : "NULL";
    ...
    this.db.exec(`
      ...
      SELECT
        legacy.id,
        legacy.fact,
        COALESCE(legacy.tags, '[]'),
        legacy.time,
        COALESCE(${has("timeliness") ? "legacy.timeliness" : "'persistent'"}, 'persistent'),
        ${has("state_key") ? "legacy.state_key" : "NULL"},
        ${has("valid_from") ? "legacy.valid_from" : "COALESCE(legacy.time, legacy.created_at)"},
        ${has("valid_to") ? "legacy.valid_to" : "NULL"},
        CASE
          WHEN ${has("is_active") ? "COALESCE(legacy.is_active, 1)" : "1"} = 0 THEN 0
          WHEN ${has("valid_to") ? "legacy.valid_to IS NOT NULL AND legacy.valid_to < CURRENT_TIMESTAMP" : "0"} THEN 0
          ELSE 1
        END,
        ${has("session_id") ? "legacy.session_id" : "NULL"},
        COALESCE(legacy.created_at, CURRENT_TIMESTAMP),
        COALESCE(${has("scope") ? "legacy.scope" : "'agent'"}, 'agent'),
        COALESCE(${has("origin") ? "legacy.origin" : "'import'"}, 'import'),
        COALESCE(${has("source_refs") ? "legacy.source_refs" : "'[]'"}, '[]'),
        ${has("time") ? "legacy.time" : "NULL"},
        ${has("confidence") ? "legacy.confidence" : "NULL"},
        ${has("importance") ? "legacy.importance" : "NULL"},
        ${has("subject_id") ? "legacy.subject_id" : "NULL"},
        CASE
          WHEN ${legacyMemoryKind} IS NOT NULL THEN ${legacyMemoryKind}
          WHEN COALESCE(${has("timeliness") ? "legacy.timeliness" : "'persistent'"}, 'persistent') = 'stateful' THEN 'state'
          WHEN COALESCE(${has("scope") ? "legacy.scope" : "'agent'"}, 'agent') = 'profile'
               AND COALESCE(${has("timeliness") ? "legacy.timeliness" : "'persistent'"}, 'persistent') = 'persistent' THEN 'profile_preference'
          ELSE 'background'
        END,
        ${has("decision_key") ? "legacy.decision_key" : "NULL"},
        ${has("staleness_hint") ? "legacy.staleness_hint" : "NULL"},
        COALESCE(${has("hash") ? "legacy.hash" : "''"}, ''),
        COALESCE(${has("updated_at") ? "legacy.updated_at" : "legacy.created_at"}, CURRENT_TIMESTAMP),
        ${has("invalidated_by") ? "legacy.invalidated_by" : "NULL"}
      FROM facts_legacy_v2 legacy
      WHERE NOT EXISTS (
        SELECT 1
        FROM facts
    ...
    const missingAfter = this.db.prepare(`
      SELECT COUNT(*) AS cnt
      FROM facts_legacy_v2 legacy
      WHERE NOT EXISTS (
        SELECT 1
    ...
    `).get()?.cnt || 0;
    if (missingAfter !== 0) {
      throw new Error(`facts resume migration mismatch: missing=${missingAfter}`);
    }
  }

  _ensureEvidenceColumns() {
    const hasEvidence = this._tableExists("evidence");
    if (!hasEvidence) return;
    this._ensureColumn("evidence", "retention_class", "TEXT NOT NULL DEFAULT 'session'");
    this._ensureColumn("evidence", "archive_after", "TEXT");
    this._ensureColumn("evidence", "purge_after", "TEXT");
    this.db.exec(`
      UPDATE evidence
      SET retention_class = COALESCE(NULLIF(TRIM(retention_class), ''), 'session')
    `);
    this.db.exec(`
      CREATE INDEX IF NOT EXISTS idx_evidence_archive_after ON evidence(archive_after, created_at DESC)
    `);
  ...
  _ensureEpisodeColumns() {
    const hasEpisodes = this._tableExists("episodes");
  ...
  _ensurePlaybookColumns() {
    const hasPlaybooks = this._tableExists("playbooks");
