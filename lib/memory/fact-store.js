...
export class FactStore {
      ...
      while (v < SCHEMA_VERSION) {
        switch (v) {
          ...
          case 1: {
            ...
            safeExec(`ALTER TABLE facts ADD COLUMN timeliness TEXT NOT NULL DEFAULT 'persistent'`);
            safeExec(`ALTER TABLE facts ADD COLUMN state_key TEXT`);
            safeExec(`ALTER TABLE facts ADD COLUMN valid_from TEXT`);
            safeExec(`ALTER TABLE facts ADD COLUMN valid_to TEXT`);
            safeExec(`ALTER TABLE facts ADD COLUMN is_active INTEGER NOT NULL DEFAULT 1`);
            safeExec(`CREATE INDEX IF NOT EXISTS idx_facts_state_key ON facts(state_key)`);
            safeExec(`CREATE INDEX IF NOT EXISTS idx_facts_valid_to ON facts(valid_to)`);
            safeExec(`CREATE INDEX IF NOT EXISTS idx_facts_is_active ON facts(is_active)`);
            // 老数据统一视为 active/persistent；若已有失效时间则同步 active 标记。
            safeExec(`UPDATE facts SET timeliness = 'persistent' WHERE timeliness IS NULL OR timeliness = ''`);
            safeExec(`UPDATE facts SET is_active = CASE
  ...
  _prepareStatements() {
    this._stmts = {
      insert: this.db.prepare(`
        INSERT INTO facts (
          fact, tags, time, timeliness, state_key, valid_from, valid_to, is_active, session_id, created_at
        )
        VALUES (
          @fact, @tags, @time, @timeliness, @stateKey, @validFrom, @validTo, @isActive, @sessionId, @createdAt
        )
      `),
      deactivateByStateKey: this.db.prepare(`
        ...
        WHERE state_key = @stateKey AND is_active = 1
      `),
      ...
      ftsSearch: this.db.prepare(`
        ...
        LIMIT ?
      `),
      ftsSearchActive: this.db.prepare(`
        SELECT f.*, rank
        FROM facts_fts fts
        JOIN facts f ON f.id = fts.rowid
        WHERE facts_fts MATCH ?
    ...
    if (stateKey) {
      ...
      if (tags.length >= 2) {
        const legacy = this.searchByTags(tags, undefined, 100, { includeInactive: true })
          .filter((r) => r.is_active && !r.state_key && (r.matchCount || 0) >= 2);
        for (const r of legacy) {
          this._stmts.deactivateById.run({ id: r.id, validTo: validFrom || now });
        }
      }
    }
  ...
  searchFullText(query, limit = 20, opts = {}) {
    ...
    try {
      ...
      // FTS 查询语法错误时降级为 LIKE
      return this._likeFallback(query, limit, { includeInactive });
    }
  }
  ...
  _likeFallback(query, limit, opts = {}) {
    ...
    const nowIso = new Date().toISOString();
    const rows = includeInactive
      ? this.db.prepare(`SELECT * FROM facts WHERE fact LIKE '%' || ? || '%' ORDER BY time DESC LIMIT ?`).all(query, limit)
      : this.db.prepare(`
        SELECT *
        FROM facts
        WHERE fact LIKE '%' || ? || '%'
  ...
  /**
   * 批量导入
   * @param {Array<{ fact, tags, time?, session_id?, timeliness?, state_key?, ttl_days?, valid_from?, valid_to? }>} entries
   */
  importAll(entries) {
    const run = this.db.transaction(() => {
      for (const entry of entries) {
