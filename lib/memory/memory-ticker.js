 ...
 * - 每天一次（日期变化时触发）：compileWeek + compileLongterm + deep-memory + compileFacts + assemble
 *
...
import { resolveClaudeTranscriptPath } from "../../core/claude-transcript.js";
import {
  compileToday,
  compileWeek,
  compileLongterm,
...
const TRIGGER_DELAY_MS = 3_000; // 到达触发点后统一延迟，降低突发并发

// ── session JSONL 解析 ──
const SESSION_METADATA_EXT = ".session.json";

/**
 * 从 session 元数据或 transcript 文件提取消息列表（带时间戳）
 */
...
function readSessionMetadataLite(sessionPath) {
  try {
...
function resolveSessionSource(sessionPath) {
  const inputPath = String(sessionPath || "").trim();
  if (!inputPath) return { sessionId: "", messagePath: null };

  // 旧格式：直接是 .jsonl 会话文件
  if (inputPath.endsWith(".jsonl")) {
    return {
      sessionId: sessionIdFromFilename(path.basename(inputPath)),
      messagePath: inputPath,
    };
  }

  // 新格式：.session.json 元数据 + projects/*/<sessionId>.jsonl transcript
  if (inputPath.endsWith(SESSION_METADATA_EXT)) {
    const meta = readSessionMetadataLite(inputPath);
    const sessionId = String(meta?.sessionId || sessionIdFromFilename(path.basename(inputPath)));
    const cwd = String(meta?.cwd || "");

    // 兼容：部分场景会把 message log 与 metadata 放在同目录
    const localJsonl = inputPath.replace(/\.session\.json$/i, ".jsonl");
    if (fs.existsSync(localJsonl)) {
      return { sessionId, messagePath: localJsonl };
    }

    const transcriptPath = resolveClaudeTranscriptPath(sessionId, cwd);
    return { sessionId, messagePath: transcriptPath || null };
  }
...
function readSessionMessages(sessionPath) {
  const { messagePath } = resolveSessionSource(sessionPath);
  if (!messagePath) return { messages: [], lastTimestamp: null };

  let raw;
  try {
    const stat = fs.statSync(messagePath);
    if (stat.size > TAIL_READ_THRESHOLD) {
      ...
      raw = fs.readFileSync(messagePath, "utf-8");
    }
  } catch {
  ...
  const pushMessage = (msg, timestamp) => {
    const role = String(msg?.role || "");
    if (role !== "user" && role !== "assistant") return;
    messages.push({ role, content: msg.content, timestamp: timestamp || null });
  ...
  for (const line of raw.split("\n")) {
    ...
    try {
      ...
      if (entry?.type === "context_reset" && Array.isArray(entry.messages)) {
        messages.length = 0;
        for (const m of entry.messages) pushMessage(m, timestamp);
        continue;
      }

      // 兼容旧格式：{ type: "message", message: { role, content } }
      // 兼容新格式：{ type: "user"|"assistant", message: { role, content } }
      if (entry?.message) {
        const role = entry?.message?.role || (entry.type === "user" || entry.type === "assistant" ? entry.type : "");
        if (role === "user" || role === "assistant") {
          pushMessage({ role, content: entry.message.content }, timestamp);
          continue;
        }
      }

      // 极端兜底：整行就是 message 对象
      if (entry?.role === "user" || entry?.role === "assistant") {
        pushMessage(entry, timestamp);
      }
    } catch {
      // 跳过损坏行
    }
...
function deriveRoleFromEvidence(row) {
  const sourceType = String(row?.sourceType || "").trim().toLowerCase();
  if (sourceType.startsWith("user_")) return "user";
  if (sourceType === "channel_messages" || sourceType === "session_message") return "user";
  return "assistant";
}

function buildMessagesFromEvidence(rows) {
  const ordered = [...rows].sort((a, b) => {
    const am = _toEpochMs(a?.updatedAt || a?.createdAt);
    const bm = _toEpochMs(b?.updatedAt || b?.createdAt);
    return am - bm;
  });
  const messages = [];
  let lastTimestamp = null;
  for (const row of ordered) {
    const content = String(row?.content || row?.preview || "").trim();
    if (!content) continue;
    const timestamp = String(row?.updatedAt || row?.createdAt || "").trim() || null;
    const role = deriveRoleFromEvidence(row);
...
/**
 * 列出可用于补偿扫描的 session 文件（.session.json / .jsonl）
 */
  ...
  function scanDir(dir, prefix) {
    try {
      for (const f of fs.readdirSync(dir)) {
        if (!f.endsWith(".jsonl") && !f.endsWith(SESSION_METADATA_EXT)) continue;
        const fp = path.join(dir, f);
        try {
          const stat = fs.statSync(fp);
          if (!stat.isFile()) continue;

          const { sessionId, messagePath } = resolveSessionSource(fp);
          let mtime = stat.mtime;
          if (messagePath) {
            try {
              mtime = fs.statSync(messagePath).mtime;
            } catch {
              // ignore message path stat errors, fallback to metadata mtime
            }
          }

          const key = sessionId || sessionIdFromFilename(f);
          if (!key) continue;
          const next = {
          ...
          if (!prev || next.mtime.getTime() >= prev.mtime.getTime()) {
            bySessionId.set(key, next);
          }
        } catch {}
      }
    } catch {}
 ...
 * @param {import('./memory-service.js').MemoryService|null} [opts.memoryService]
...
export function createMemoryTicker(opts) {
  ...
  const _delayedRollingTimers = new Map();   // sessionPath -> timeout id
  let _dailyDelayTimer = null;

  // ── 内部：滚动摘要 ──

  async function _doRollingSummary(sessionPath, { allowTranscriptRecovery = false, recoveryReason = "" } = {}) {
    if (_summaryInProgress.has(sessionPath)) return; // 并发保护
    _summaryInProgress.add(sessionPath);
    try {
      const { sessionId } = resolveSessionSource(sessionPath);
      if (!sessionId) return;
      let messages = [];
      let lastTimestamp = null;
      let inputSource = "evidence";

      if (memoryService) {
        const evidenceRows = memoryService.listEvidenceBySession(sessionId);
        const built = buildMessagesFromEvidence(evidenceRows);
        messages = built.messages;
        lastTimestamp = built.lastTimestamp;
      }

      if (messages.length === 0 && allowTranscriptRecovery) {
        const fallback = readSessionMessages(sessionPath);
        messages = fallback.messages;
        lastTimestamp = fallback.lastTimestamp;
        inputSource = "transcript_recovery";
        memoryService?.logDiagnostic("rolling_summary_transcript_recovery", {
          sessionId,
          reason: recoveryReason || "recovery",
          messageCount: messages.length,
          lastTimestamp,
        });
      }
      if (messages.length === 0) {
        memoryService?.logDiagnostic("missing_evidence_for_rolling_summary", {
          sessionId,
      ...
      if (memoryService) {
        ...
        if (evidenceRows.length > 0) {
          const summary = summaryManager.getSummary(sessionId)?.summary || "";
          ...
          memoryService.logDiagnostic("missing_turn_evidence_for_summary", {
            sessionId,
            messageCount: messages.length,
            lastTimestamp,
            inputSource,
  ...
  async function _doCompileTodayAndAssemble() {
    try {
      ...
      const content = assemble(factsMdPath, todayMdPath, weekMdPath, longtermMdPath, memoryMdPath);
      memoryService?.setSummaryProjection(content, { sourceScope: "agent" });
      onCompiled?.();
  ...
  async function _doExtractExperiences(sessionPath) {
    ...
    try {
      ...
      const episode = memoryService.getEpisodeBySession(sessionId);
      const episodeAnchor = episode?.anchorText || summaryManager.getSummary(sessionId)?.summary || "";
      if (!episodeAnchor.trim()) return;

      const { extracted } = await extractSessionExperiences(
        {
          episodeAnchor,
          evidenceRows: evidenceRows.slice(0, 3),
          memoryService,
          resolvedModel: getResolvedMemoryModel(),
          origin: String(sessionId || "").startsWith("channel-") ? "channel" : "session",
        },
      );
      if (extracted > 0) {
  ...
  async function _doDaily() {
    ...
    try {
      ...
      // Step 3: deep-memory（更新 facts.db，必须先于 compileFacts）
      if (!_dailyStepsCompleted.has("deepMemory")) {
        try {
          const { processed, factsAdded } = await processDirtySessions(
            summaryManager, memoryService || factStore, getResolvedMemoryModel(),
          );
      ...
      // Step 4: compileFacts（读取 deep-memory 已写入的 facts.db）
      if (!_dailyStepsCompleted.has("compileFacts")) {
        try {
          await compileFacts(summaryManager, factsMdPath, getResolvedMemoryModel(), {
            factStore,
            sourceScope: "agent",
            playbooks: memoryService?.listPlaybooks?.({ activeOnly: true, scope: "agent" }) || [],
          });
      ...
      // Step 5: assemble（纯文件操作，用已有的 .md 文件组装，总是执行）
      try {
        const content = assemble(factsMdPath, todayMdPath, weekMdPath, longtermMdPath, memoryMdPath);
        memoryService?.setSummaryProjection(content, { sourceScope: "agent" });
        onCompiled?.();
  ...
  function _scheduleRollingSummary(sessionPath) {
    if (!sessionPath || _delayedRollingTimers.has(sessionPath)) return;
    const timer = setTimeout(() => {
      _delayedRollingTimers.delete(sessionPath);
      _doRollingSummary(sessionPath)
        .then(() => _doCompileTodayAndAssemble())
        .catch(() => {});
    }, TRIGGER_DELAY_MS);
    if (timer.unref) timer.unref();
    _delayedRollingTimers.set(sessionPath, timer);
  }

  function _scheduleDailyCheck() {
    if (_dailyDelayTimer) return;
    _dailyDelayTimer = setTimeout(() => {
      _dailyDelayTimer = null;
      _checkDailyJob();
    }, TRIGGER_DELAY_MS);
    if (_dailyDelayTimer.unref) _dailyDelayTimer.unref();
  }
   ...
   * @param {string} sessionPath - 当前 session 的 .session.json 或 .jsonl 路径
   */
  function notifyTurn(sessionPath) {
    ...
    if (count % TURNS_PER_SUMMARY === 0 && memoryOn) {
      _scheduleRollingSummary(sessionPath);
    }

    if (memoryOn) _scheduleDailyCheck();
  }
  ...
  async function notifySessionEnd(sessionPath) {
    ...
    const delayedTimer = _delayedRollingTimers.get(sessionPath);
    if (delayedTimer) {
  ...
  function start() {
    if (_timer) return;
    _timer = setInterval(() => _scheduleDailyCheck(), DAILY_CHECK_INTERVAL);
  ...
  async function stop() {
    ...
    if (_dailyDelayTimer) {
      clearTimeout(_dailyDelayTimer);
      _dailyDelayTimer = null;
    }
    for (const timer of _delayedRollingTimers.values()) {
      clearTimeout(timer);
    }
    _delayedRollingTimers.clear();
  ...
  async function _recoverUnsummarized() {
    ...
    for (const { filePath, mtime, sessionId } of sessions) {
