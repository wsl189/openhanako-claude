...
function normalizeMemoryPayload(rawMemory) {
  ...
  const rawAction = toText(rawMemory.action || "archive").toLowerCase();
  if (rawAction !== "archive" && rawAction !== "clear") {
    throw new Error("memory.action must be archive");
  }
  ...
  return {
    ...
    include_compiled: rawMemory.include_compiled === true,
...
export function createSetupSettingsTool({ engine }) {
  return {
    ...
    description: "Apply Hanako-only setup changes from one tutorial JSON or explicit fields (never configure external products). Supports: agent create/update/delete (profile, model/workspace, tool permissions, identity/ishiki), skill install from local SKILL.md directory (global + optional enable for current agent), MCP registration (stdio/sse/http; include mcp.env when target MCP docs require API key/host or other env vars, and persist those vars in Hanako MCP settings instead of relying on system environment edits), memory clear for target agent (compiled files + optional pinned/permanent), and dry-run validation. After installing skill/MCP, run a smoke test; if dependencies/environment are missing, install business dependencies into Hanako shared runtime first, ensure runners such as uvx/npx/bunx are available via system PATH, then re-test.",
    parameters: Type.Object({
      ...
      mcp: Type.Optional(Type.Object({
        ...
        env: Type.Optional(Type.Record(Type.String(), Type.String(), { description: "Stdio env map. Required when the target MCP's docs require environment variables (for example API keys/hosts). Persist these vars here so they appear in Hanako MCP settings; do not rely on editing system environment variables as a substitute." })),
      ...
      memory: Type.Optional(Type.Object({
        action: Type.Optional(Type.Union([
          ...
          Type.Literal("clear"),
        ], { description: "Memory action. archive is preferred; clear is accepted as a legacy alias." })),
        agent_id: Type.Optional(Type.String({ description: "Target agent id. Omit to use current agent." })),
        include_compiled: Type.Optional(Type.Boolean({ default: false, description: "Legacy alias. Current memory summary is read-only, so compiled projections are not cleared." })),
        include_pinned: Type.Optional(Type.Boolean({ default: false, description: "Whether to also archive high-priority memory marks." })),
      })),
    ...
    execute: async (_toolCallId, params) => {
      try {
        ...
        if (spec.memory && typeof spec.memory === "object") {
          ...
          const targetFactsDbPath = path.join(targetAgentDir, "memory", "facts.db");
          ...
          if (!dryRun) {
            const service = targetAgentId === activeAgentId
            ...
            try {
              const archiveIds = service.listArchiveCandidateIds({
                includeFacts: true,
                includePlaybooks: true,
                includeMarks: memory.include_pinned,
              });
              service.archive(archiveIds);
            } finally {
              if (service !== engine.memoryService) service.close();
            }
          ...
          if (memory.include_compiled) {
            details.checks.push({
              type: "memory_summary_projection_read_only",
              ok: true,
              ignored: true,
              reason: "current memory summary is a read-only projection and is no longer cleared",
            });
          }
