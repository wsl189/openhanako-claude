...
export function createSetupSettingsTool({ engine }) {
  return {
    ...
    description: "Apply Hanako-only setup changes from one tutorial JSON or explicit fields (never configure external products). Supports: agent create/update/delete (profile, model/workspace, tool permissions, identity/ishiki), skill install from local SKILL.md directory (global + optional enable for current agent), MCP registration (stdio/sse/http), memory clear for target agent (compiled files + optional pinned/permanent), and dry-run validation.",
    parameters: Type.Object({
      ...
      mcp: Type.Optional(Type.Object({
        ...
        env: Type.Optional(Type.Record(Type.String(), Type.String(), { description: "Optional stdio env map." })),
    ...
    execute: async (_toolCallId, params) => {
      try {
        ...
        if (spec.agent && typeof spec.agent === "object") {
          const action = toText(spec.agent.action || "update").toLowerCase() || "update";
