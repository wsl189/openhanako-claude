...
function normalizeLegacyMcpServers(rawMcpServers) {
  if (!rawMcpServers || typeof rawMcpServers !== "object" || Array.isArray(rawMcpServers)) return null;
  const entries = Object.entries(rawMcpServers).filter(([k, v]) => toText(k) && v && typeof v === "object" && !Array.isArray(v));
  if (!entries.length) return null;
  const [name, config] = entries[0];
  const normalizedName = toText(config?.name) || toText(name);
  return {
...
function buildMergedSpec(params) {
  ...
  return {
    ...
    mcp: (params?.mcp && typeof params.mcp === "object")
      ? params.mcp
      : ((parsed?.mcp && typeof parsed.mcp === "object") ? parsed.mcp : (legacyMcpFromParams || legacyMcpFromTutorial)),
