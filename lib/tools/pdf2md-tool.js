...
function resolveCacheDir(ctx) {
  const cwd = ctx?.sessionManager?.getCwd?.() || process.cwd();
  return path.join(cwd, ".hanako", "pdf2md");
}

function resolveCachePath(filePath, ctx) {
  return path.join(resolveCacheDir(ctx), `${safeOutputStem(filePath)}.md`);
}

function resolveOutputPath(filePath, requested, ctx) {
  const explicit = sanitizeInputPath(requested || "");
  if (explicit) return path.resolve(explicit);
  return resolveCachePath(filePath, ctx);
}

function cleanupCacheDir(cacheDir, ttlMs = DEFAULT_CACHE_TTL_MS) {
  const safeTtl = Number.isFinite(ttlMs) && ttlMs > 0 ? ttlMs : DEFAULT_CACHE_TTL_MS;
  ...
  for (const entry of entries) {
    ...
    if (!st?.mtimeMs || now - st.mtimeMs < safeTtl) continue;
    try {
...
function readCachedMarkdown(cachePath) {
...
export function createPdf2MdTool({ env = process.env, fetchImpl = globalThis.fetch, getConfig = () => ({}) } = {}) {
  return {
    ...
    execute: async (_toolCallId, params, signal, _messages, ctx) => {
      ...
      const cacheTtlMs = Number.parseInt(config.cache_ttl_ms || config.cacheTtlMs || env.HANA_PDF2MD_CACHE_TTL_MS || "", 10);
      const cachePruned = cleanupCacheDir(cacheDir, cacheTtlMs);
      const cachePath = resolveCachePath(filePath, ctx);
      const outputPath = resolveOutputPath(filePath, params.output_path, ctx);
      const cachedMarkdown = readCachedMarkdown(cachePath);
      if (cachedMarkdown.trim()) {
        if (outputPath !== cachePath) {
          fs.mkdirSync(path.dirname(outputPath), { recursive: true });
          fs.writeFileSync(outputPath, cachedMarkdown, "utf-8");
        }
        const { preview, truncated } = buildPreview(cachedMarkdown, params.max_preview_chars);
        const header = t("toolDef.pdf2md.cacheHit", {
          cachePath,
          outputPath,
        });
        ...
        return {
          ...
          details: {
            ...
            cachePath,
            sourcePdf: filePath,
            markdownChars: cachedMarkdown.length,
            cacheHit: true,
            cachePruned,
          },
        };
      ...
      try {
        ...
        return {
          ...
          details: { error: "call_failed", message: msg, cachePruned },
        };
      } finally {
