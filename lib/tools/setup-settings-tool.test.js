...
describe("setup-settings-tool", () => {
  it("documents MCP env requirement in setup_settings schema", () => {
    const root = mktemp("setup-settings-tool-schema-");
    ...
    const envDesc = String(tool.parameters?.properties?.mcp?.properties?.env?.description || "");
    expect(desc).toContain("include mcp.env when target MCP docs require API key/host");
    expect(desc).toContain("persist those vars in Hanako MCP settings");
    expect(desc).toContain("After installing skill/MCP, run a smoke test");
    expect(desc).toContain("install business dependencies into Hanako shared runtime first");
    expect(desc).toContain("ensure runners such as uvx/npx/bunx are available via system PATH");
    expect(envDesc).toContain("Required when the target MCP's docs require environment variables");
    expect(envDesc).toContain("do not rely on editing system environment variables");
  });
  ...
  it("accepts legacy mcpServers payload from tutorial json", async () => {
    const root = mktemp("setup-settings-tool-legacy-mcp-tutorial-");
    let mcpServers = {};
    const tool = createSetupSettingsTool({
      engine: {
        hanakoHome: root,
        ...
        agentDir: path.join(root, "agents", "hanako"),
        agentsDir: path.join(root, "agents"),
        patchExternalMcpServers: vi.fn((patch) => { mcpServers = { ...mcpServers, ...patch }; }),
        refreshCurrentSessionTools: vi.fn(async () => {}),
      },
    });
    const result = await tool.execute("tc-legacy-tutorial", {
      tutorial: JSON.stringify({
        mcpServers: {
          MiniMax: {
            command: "sh",
            args: ["-lc", "echo minimax"],
            env: {
              MINIMAX_API_KEY: "k",
    ...
    expect(result?.content?.[0]?.text || "").toContain("Setup applied");
    expect(result?.details?.applied?.mcp).toMatchObject({
      name: "MiniMax",
      config: {
        type: "stdio",
        command: "sh",
    ...
    expect(mcpServers.MiniMax).toMatchObject({
      type: "stdio",
      command: "sh",
      args: ["-lc", "echo minimax"],
      env: {
  ...
  it("accepts legacy mcpServers payload as direct params", async () => {
    const root = mktemp("setup-settings-tool-legacy-mcp-direct-");
    let mcpServers = {};
    const tool = createSetupSettingsTool({
      engine: {
        hanakoHome: root,
    ...
    const result = await tool.execute("tc-legacy-direct", {
      mcpServers: {
        MiniMax: {
          command: "sh",
          args: ["-lc", "echo minimax"],
    ...
    expect(result?.content?.[0]?.text || "").toContain("Setup applied");
    expect(result?.details?.applied?.mcp?.name).toBe("MiniMax");
    expect(mcpServers.MiniMax?.command).toBe("sh");
  });

  it("installs all servers when legacy mcpServers contains multiple entries", async () => {
    const root = mktemp("setup-settings-tool-legacy-mcp-multi-");
    let mcpServers = {};
    const tool = createSetupSettingsTool({
      engine: {
        hanakoHome: root,
        ...
        agentsDir: path.join(root, "agents"),
        patchExternalMcpServers: vi.fn((patch) => { mcpServers = { ...mcpServers, ...patch }; }),
        refreshCurrentSessionTools: vi.fn(async () => ({ reloaded: true })),
      },
    });

    const result = await tool.execute("tc-legacy-multi", {
      mcpServers: {
        Alpha: { command: "sh", args: ["-lc", "echo alpha"] },
        Beta: { command: "sh", args: ["-lc", "echo beta"] },
      },
    });

    expect(result?.content?.[0]?.text || "").toContain("Setup applied");
    expect(result?.details?.applied?.mcp?.count).toBe(2);
    expect(Array.isArray(result?.details?.applied?.mcp?.servers)).toBe(true);
    expect(mcpServers.Alpha?.command).toBe("sh");
    expect(mcpServers.Beta?.command).toBe("sh");
  });

  it("accepts case-insensitive MCP type", async () => {
    const root = mktemp("setup-settings-tool-mcp-type-case-");
    let mcpServers = {};
    const tool = createSetupSettingsTool({
      engine: {
        hanakoHome: root,
    ...
    const result = await tool.execute("tc-mcp-type-case", {
      mcp: {
        name: "CaseType",
        type: "STDIO",
        command: "sh",
    ...
    expect(result?.content?.[0]?.text || "").toContain("Setup applied");
    expect(mcpServers.CaseType?.type).toBe("stdio");
  });

  it("keeps quoted args together when mcp.args is a string", async () => {
    const root = mktemp("setup-settings-tool-mcp-args-string-");
    let mcpServers = {};
    const tool = createSetupSettingsTool({
      engine: {
        hanakoHome: root,
    ...
    const result = await tool.execute("tc-mcp-args-string", {
      mcp: {
        name: "QuotedArgs",
        command: "sh",
    ...
    expect(result?.content?.[0]?.text || "").toContain("Setup applied");
    expect(mcpServers.QuotedArgs?.args).toEqual(["-lc", "echo hello world"]);
  });

  it("fails when mcp.command is missing from PATH", async () => {
    const root = mktemp("setup-settings-tool-mcp-missing-cmd-");
    const tool = createSetupSettingsTool({
      engine: {
        hanakoHome: root,
        userSkillsDir: path.join(root, "skills"),
    ...
    const result = await tool.execute("tc-mcp-missing-cmd", {
      mcp: {
        name: "MissingCmd",
        command: "__def_not_exists_foo__",
      },
    });

    expect(result?.content?.[0]?.text || "").toContain("setup_settings failed");
    expect(result?.content?.[0]?.text || "").toContain("mcp.command not found in PATH");
  });

  it("reports deferred MCP tool reload when session is streaming", async () => {
    const root = mktemp("setup-settings-tool-mcp-streaming-refresh-");
    const tool = createSetupSettingsTool({
      engine: {
        hanakoHome: root,
        userSkillsDir: path.join(root, "skills"),
        ...
        agentsDir: path.join(root, "agents"),
        patchExternalMcpServers: vi.fn(() => {}),
        refreshCurrentSessionTools: vi.fn(async () => ({ reloaded: false, reason: "streaming" })),
      },
    });

    const result = await tool.execute("tc-mcp-streaming-refresh", {
      mcp: {
        name: "RefreshPending",
        command: "sh",
      },
    ...
    expect(result?.content?.[0]?.text || "").toContain("MCP config was saved, but current session is streaming");
    const refreshCheck = (result?.details?.checks || []).find((item) => item.type === "mcp_tools_refresh");
    expect(refreshCheck?.ok).toBe(false);
    expect(refreshCheck?.reason).toBe("streaming");
  });
  ...
  it("archives memory and pinned memory through memory service", async () => {
    ...
    fs.writeFileSync(path.join(activeAgentDir, "memory", "memory.md"), "memory content", "utf-8");
    fs.writeFileSync(path.join(activeAgentDir, "memory", "today.md"), "today content", "utf-8");
    fs.writeFileSync(path.join(activeAgentDir, "pinned.md"), "- keep this\n", "utf-8");
    const archive = vi.fn();
    const listArchiveCandidateIds = vi.fn(() => ["fact:11", "playbook:playbook-1", "mark:mark-1"]);
    const memoryService = {
    ...
    expect(result?.content?.[0]?.text || "").toContain("Setup applied");
    expect(listArchiveCandidateIds).toHaveBeenCalledWith({
      includeFacts: true,
      includePlaybooks: true,
      includeMarks: true,
    });
    expect(archive).toHaveBeenCalledWith(["fact:11", "playbook:playbook-1", "mark:mark-1"]);
    expect(result?.details?.applied?.memory?.action).toBe("archive");
    expect(fs.readFileSync(path.join(activeAgentDir, "memory", "memory.md"), "utf-8")).toBe("memory content");
    expect(fs.readFileSync(path.join(activeAgentDir, "memory", "today.md"), "utf-8")).toBe("today content");
  });
