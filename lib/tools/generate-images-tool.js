  ...
  return {
    ...
    execute: async (_toolCallId, params, _signal, _onUpdate, ctx) => {
      ...
      try {
        ...
        return {
          content: [{ type: "text", text: err?.message || t("error.imageGenProviderNotMinimax") }],
      ...
      if (!provider.startsWith("minimax")) {
        return {
          content: [{ type: "text", text: t("error.imageGenProviderNotMinimax") }],
          details: { error: "provider_not_minimax", provider: resolved?.provider || "" },
        };
      }
      ...
      try {
        const imageModel = "image-01";
        const imageApi = "openai-completions";
        const imageBaseUrl = resolveMinimaxImageBaseUrl(resolved.base_url);
        const generatedList = await callProviderImageGeneration({
          api: imageApi,
        ...
        const writtenFiles = generatedList.map((raw, idx) => {
          ...
          const fileName = `minimax-${mode}-${ts}-${String(idx + 1).padStart(2, "0")}.png`;
