...
const { isPersistentAgentSessionBusyMock, runAgentSessionMock } = vi.hoisted(() => ({
  isPersistentAgentSessionBusyMock: vi.fn(),
  runAgentSessionMock: vi.fn(),
}));

vi.mock("../../hub/agent-executor.js", () => ({
  isPersistentAgentSessionBusy: isPersistentAgentSessionBusyMock,
...
describe("ask_agent tool channel posting", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    isPersistentAgentSessionBusyMock.mockReturnValue(false);
  ...
  it("posts reply as target agent when channel is provided", async () => {
    ...
    expect(result?.details?.postedToChannel).toBe(false);
    expect(result?.details?.async).toBe(true);
    expect(result?.details?.channel).toBe("ch_team");
    expect(result?.content?.[0]?.text).toContain("已派发");
    await vi.waitFor(() => expect(runAgentSessionMock).toHaveBeenCalledTimes(1));
  ...
  it("queues channel ask when target agent session is already busy", async () => {
    const channelsDir = path.join(tempRoot, "channels");
    ...
    appendMessage(filePath, "user", "请大家处理");
    isPersistentAgentSessionBusyMock.mockReturnValue(true);
    runAgentSessionMock.mockResolvedValue("beta async done");
    ...
    const result = await tool.execute("tc-busy", {
      agent: "beta",
      task: "稍后处理",
      channel: "ch_team",
    });
    ...
    expect(result.details.async).toBe(true);
    expect(result.content[0].text).toContain("已排队");
    await vi.waitFor(() => expect(runAgentSessionMock).toHaveBeenCalledTimes(1));
    const content = fs.readFileSync(filePath, "utf-8");
    expect(content).toContain("beta async done");
  });
  ...
  it("injects channel role context and recent messages into asked agent prompt", async () => {
    ...
    expect(roundText).toContain("少量最近频道消息");
    expect(roundText).toContain("用户问题1");
    expect(roundText).toContain("协调说明 a1");
    expect(roundText).not.toContain("较新回复 b2");
    expect(roundText).not.toContain("最新回复 b3");
    expect(roundText).not.toContain("旧回复 b1");
  });
  ...
  it("runs in parallel when multiple agents are provided and emits activity events", async () => {
    ...
    expect(result?.content?.[0]?.text).toContain("频道派工完成");
    expect(result?.details?.success).toBe(2);
    expect(result?.details?.failed).toBe(0);
    expect(result?.details?.results?.every((r) => r.async)).toBe(true);
    await vi.waitFor(() => expect(runAgentSessionMock).toHaveBeenCalledTimes(2));
  ...
  it("does not make a regular multi-agent delegation a supervisor follow-up", async () => {
    const channelsDir = path.join(tempRoot, "channels");
    createChannel(channelsDir, { id: "ch_team", name: "team", members: ["alpha", "beta", "gamma"] });
    runAgentSessionMock.mockImplementation(async (targetId) => `${targetId} done`);

    const triggerChannelTriage = vi.fn();
    const engine = {
      channelsDir,
      _hub: {
        eventBus: { emit: vi.fn() },
        triggerChannelTriage,
      },
    };
    const tool = createAskAgentTool({
      agentId: "alpha",
    ...
    const result = await tool.execute("tc-no-supervisor", {
      agents: ["beta", "gamma"],
      task: "普通协作，不要监督",
      channel: "ch_team",
    });

    expect(result?.details?.supervising).toBe(false);
    await vi.waitFor(() => expect(runAgentSessionMock).toHaveBeenCalledTimes(2));
    await new Promise((resolve) => setTimeout(resolve, 0));
    expect(triggerChannelTriage).not.toHaveBeenCalled();
  });

  it("wakes a channel leader after delegated reports", async () => {
    const channelsDir = path.join(tempRoot, "channels");
    ...
    runAgentSessionMock.mockResolvedValue("beta done");
    ...
    const result = await tool.execute("tc-leader-supervisor", {
      agent: "beta",
      task: "请汇报进度",
      channel: "ch_team",
    });

    expect(result?.details?.supervising).toBe(true);
    await vi.waitFor(() => expect(triggerChannelTriage).toHaveBeenCalledWith("ch_team", expect.objectContaining({
      source: "agent",
      mentionedAgents: ["alpha"],
      reason: "ask_agent_report",
      reporterAgentId: "beta",
    })));
  });
});
