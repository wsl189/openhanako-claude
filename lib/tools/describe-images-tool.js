...
function extractImagePathsFromMessages(messages = []) {
  ...
  for (let i = list.length - 1; i >= 0; i -= 1) {
    ...
    for (const lineRaw of lines) {
      ...
      const tagged = line.match(/^\[(?:附件|attachment)\]\s+(.+)$/i);
      if (tagged?.[1]) {
        const p = tagged[1].trim().replace(/^["']|["']$/g, "");
        if (p) candidates.push(p);
        continue;
      }

      // 兜底：行内直接出现的绝对图片路径
      const absPathMatches = line.match(/(?:^|\s)(\/\S+\.(?:png|jpe?g|gif|webp|bmp|svg|ico))(?=\s|$)/ig);
      if (absPathMatches?.length) {
        for (const hit of absPathMatches) {
          const p = hit.trim().replace(/^["']|["']$/g, "");
          if (p) candidates.push(p);
        }
      }
    }
  ...
  return {
    ...
    execute: async (_toolCallId, params, _signal, _onUpdate, ctx) => {
      ...
      if (!allImages.length) {
        ...
        const messages = getSessionMessages?.(msgSessionPath) || [];
        const imagePaths = extractImagePathsFromMessages(messages);
        allImages = readImagesFromPaths(imagePaths, 10);
      }
      if (!allImages.length) {
        return {
          content: [{ type: "text", text: t("error.imageToolNoImages") }],
          details: { error: "no_images" },
        };
      }
