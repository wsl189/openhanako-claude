...
const SETTINGS_REGISTRY = {
  sandbox: {
    ...
    searchTerms: ["security", "安全", "权限"],
    scope: "agent",
    get: (engine) => String((engine.agent?.config?.sandbox?.mode || "standard") !== "full-access"),
    apply: (engine) => (v) => engine.agent?.updateConfig({ sandbox: { mode: v === "true" ? "standard" : "full-access" } }),
  },
  locale: {
    type: "list",
