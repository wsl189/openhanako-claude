 ...
 *
 * Windows 没有 OS 级沙盒（seatbelt/bwrap），bash 走兼容执行层。
 ...
 * 2. 注册表查询 Git 安装路径
 * 3. 内嵌 MinGit 的 sh.exe/ash.exe（打包进 resources/git/）
 * 4. PATH 上的 bash.exe / sh.exe / ash.exe
 * 5. MSYS2 / Cygwin
 */
function getAllShellCandidates() {
  ...
  // ── 3. 内嵌 MinGit 的 sh.exe/ash.exe ──
  if (process.resourcesPath) {
    for (const bundled of [
      {
        shell: join(process.resourcesPath, "git", "usr", "bin", "sh.exe"),
        label: "Bundled MinGit sh",
      },
      {
        shell: join(process.resourcesPath, "git", "mingw64", "bin", "ash.exe"),
        label: "Bundled MinGit BusyBox ash",
      },
    ]) {
      if (existsSync(bundled.shell)) {
        found.push({ shell: bundled.shell, args: ["-c"], label: `${bundled.label} (${bundled.shell})` });
      }
    }
  }

  // ── 4. PATH 上的 bash.exe / sh.exe / ash.exe ──
  for (const name of ["bash.exe", "sh.exe", "ash.exe"]) {
    try {
...
export const __win32ExecInternals = {
  getAllShellCandidates,
  isShellSpawnError,
  cleanShellEnv,
