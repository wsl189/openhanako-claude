/**
 * tool-wrapper.js — 工具沙盒包装
 *
 * 在工具 execute 外面套一层路径校验。
 * 被拦截时返回 LLM 可读的文本错误，不抛异常。
 *
 * macOS/Linux: bash 安全边界在 OS 沙盒（seatbelt/bwrap），preflight 只优化体验。
 * Windows: 无 OS 沙盒，bash 额外做路径提取 + PathGuard 校验作为安全层。
 */

import path from "path";
import { t } from "../../server/i18n.js";

/** 构造被拦截时返回给 LLM 的结果 */
function blockedResult(reason) {
  return {
    content: [{ type: "text", text: t("sandbox.blocked", { reason }) }],
  };
}

/** 解析工具参数中的路径为绝对路径 */
function resolvePath(rawPath, cwd) {
  if (!rawPath) return null;
  return path.isAbsolute(rawPath) ? rawPath : path.resolve(cwd, rawPath);
}

/**
 * 轻量 preflight 模式匹配
 * macOS/Linux: 体验层（OS 沙盒兜底）
 * Windows: 安全层之一（无 OS 沙盒）
 */
const PREFLIGHT_UNIX = [
  [/\bsudo\s/, () => t("sandbox.noSudo")],
  [/\bsu\s+\w/, () => t("sandbox.noSu")],
  [/\bchmod\s/, () => t("sandbox.noChmod")],
  [/\bchown\s/, () => t("sandbox.noChown")],
];

const PREFLIGHT_WIN32 = [
  [/\bdel\s+\/s/i, () => t("sandbox.noDelRecursive")],
  [/\brmdir\s+\/s/i, () => t("sandbox.noRmdirRecursive")],
  [/\breg\s+(delete|add)\b/i, () => t("sandbox.noRegEdit")],
  [/\btakeown\b/i, () => t("sandbox.noTakeown")],
  [/\bicacls\b/i, () => t("sandbox.noIcacls")],
  [/\bnet\s+(user|localgroup)\b/i, () => t("sandbox.noNetUser")],
  [/\bschtasks\s+\/create\b/i, () => t("sandbox.noSchtasks")],
  [/\bsc\s+(create|delete)\b/i, () => t("sandbox.noScService")],
  [/powershell.*-e(xecutionpolicy)?\s*(bypass|unrestricted)/i, () => t("sandbox.noPsExecutionBypass")],
  [/\bformat\s+[a-z]:/i, () => t("sandbox.noFormat")],
  [/\bbcdedit\b/i, () => t("sandbox.noBcdedit")],
  [/\bwmic\b/i, () => t("sandbox.noWmic")],
];

const PREFLIGHT_PATTERNS = process.platform === "win32"
  ? [...PREFLIGHT_UNIX, ...PREFLIGHT_WIN32]
  : PREFLIGHT_UNIX;

const SEGMENT_BREAKS = new Set([";", "&&", "||", "|", "\n"]);
const COMMAND_WRAPPERS = new Set(["command", "builtin", "env", "nohup", "time"]);
const DELETE_COMMANDS = new Set(["rm", "rmdir", "unlink", "del", "erase", "rd", "remove-item", "ri"]);
const MOVE_COMMANDS = new Set(["mv", "move", "move-item", "mi"]);
const COPY_COMMANDS = new Set(["cp", "copy", "install"]);
const TEE_COMMANDS = new Set(["tee"]);
const SHELL_LAUNCHERS = new Set([
  "sh",
  "bash",
  "zsh",
  "dash",
  "ksh",
  "cmd",
  "cmd.exe",
  "powershell",
  "powershell.exe",
  "pwsh",
  "pwsh.exe",
]);
const WINDOWS_SWITCH_COMMANDS = new Set([
  "del",
  "erase",
  "rd",
  "rmdir",
  "move",
  "cmd",
  "cmd.exe",
  "powershell",
  "powershell.exe",
  "pwsh",
  "pwsh.exe",
  "remove-item",
  "ri",
  "move-item",
  "mi",
]);
const URL_TOKEN = /^[a-z][a-z0-9+.-]*:\/\//i;
const ENV_TOKEN = /^\$(?:\{[A-Za-z_][A-Za-z0-9_]*\}|[A-Za-z_][A-Za-z0-9_]*)(?:[\\/]|$)/;

function tokenizeShell(command = "") {
  const tokens = [];
  let buf = "";
  let quote = null;
  let escaped = false;

  const pushBuf = () => {
    if (!buf.length) return;
    tokens.push(buf);
    buf = "";
  };

  for (let i = 0; i < command.length; i += 1) {
    const ch = command[i];

    if (escaped) {
      buf += ch;
      escaped = false;
      continue;
    }

    if (quote === "'") {
      if (ch === "'") quote = null;
      else buf += ch;
      continue;
    }

    if (quote === "\"") {
      if (ch === "\"") {
        quote = null;
        continue;
      }
      if (ch === "\\") {
        const next = command[i + 1];
        if (next === "\"" || next === "\\" || next === "$" || next === "`") {
          buf += next;
          i += 1;
          continue;
        }
      }
      buf += ch;
      continue;
    }

    if (ch === "\\") {
      const next = command[i + 1];
      if (next && /[\s\\'"`$|;&]/.test(next)) {
        escaped = true;
        continue;
      }
      buf += ch;
      continue;
    }

    if (ch === "'" || ch === "\"") {
      quote = ch;
      continue;
    }

    if (ch === "&" && command[i + 1] === "&") {
      pushBuf();
      tokens.push("&&");
      i += 1;
      continue;
    }
    if (ch === "|" && command[i + 1] === "|") {
      pushBuf();
      tokens.push("||");
      i += 1;
      continue;
    }
    if (ch === ";" || ch === "|" || ch === "\n") {
      pushBuf();
      tokens.push(ch);
      continue;
    }
    if (/\s/.test(ch)) {
      pushBuf();
      continue;
    }

    buf += ch;
  }

  if (escaped) buf += "\\";
  pushBuf();
  return tokens;
}

function normalizeCommandName(token = "") {
  return path.basename(String(token || "")).toLowerCase();
}

function looksLikeAssignment(token = "") {
  return /^[A-Za-z_][A-Za-z0-9_]*=.*/.test(token);
}

function isRedirectionToken(token = "") {
  if (!token) return false;
  return token === ">" || token === ">>" || token === "<" || token === "<<" || token.startsWith("2>") || token.startsWith("1>") || token.startsWith(">");
}

function resolveCommandPath(token, cwd) {
  if (!token) return null;
  const home = process.env.HOME || "";
  const expandEnv = (input) => input.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*)\}|\$([A-Za-z_][A-Za-z0-9_]*)/g, (m, a, b) => {
    const key = a || b;
    const val = process.env[key];
    return typeof val === "string" && val.length ? val : m;
  });
  let raw = expandEnv(token);
  if (raw === "~") raw = home || raw;
  else if (raw.startsWith("~/")) raw = home ? path.join(home, raw.slice(2)) : raw;
  return path.isAbsolute(raw) ? raw : path.resolve(cwd, raw);
}

function getCommandTokenIndex(tokens = []) {
  let idx = 0;
  while (idx < tokens.length && looksLikeAssignment(tokens[idx])) idx += 1;
  while (idx < tokens.length && COMMAND_WRAPPERS.has(normalizeCommandName(tokens[idx]))) idx += 1;
  return idx < tokens.length ? idx : -1;
}

function isLikelyPathToken(token = "") {
  if (!token || token === "--") return false;
  if (URL_TOKEN.test(token)) return false;
  if (looksLikeAssignment(token)) return false;
  if (isRedirectionToken(token)) return false;
  if (ENV_TOKEN.test(token)) return true;
  if (token === "." || token === ".." || token === "~") return true;
  if (token.startsWith("~/") || token.startsWith("./") || token.startsWith("../")) return true;
  if (process.platform === "win32") {
    if (/^[A-Za-z]:[\\/]/.test(token)) return true;
    if (token.startsWith(".\\") || token.startsWith("..\\") || token.startsWith("~\\")) return true;
  }
  return token.includes("/") || token.includes("\\");
}

function collectDeleteTargetsFromFind(tokens, cwd) {
  const hasDelete = tokens.includes("-delete");
  if (!hasDelete) return [];

  const roots = [];
  for (const tok of tokens) {
    if (!tok || tok === "--") continue;
    if (tok === "(" || tok === ")" || tok === "!" || tok.startsWith("-")) break;
    if (isRedirectionToken(tok)) break;
    roots.push(tok);
  }

  const effectiveRoots = roots.length ? roots : ["."];
  return effectiveRoots
    .map((tok) => resolveCommandPath(tok, cwd))
    .filter(Boolean);
}

function isOptionTokenForCommand(cmd = "", token = "") {
  if (!token) return false;
  if (token.startsWith("-")) return true;
  return WINDOWS_SWITCH_COMMANDS.has(cmd) && /^\/[A-Za-z?]/.test(token);
}

function materializeNestedTokens(tokens = []) {
  if (!Array.isArray(tokens) || tokens.length === 0) return [];
  if (tokens.length === 1 && /[\s;&|]/.test(tokens[0])) {
    return tokenizeShell(tokens[0]);
  }
  return tokens;
}

function extractNestedCommandTokens(cmd = "", args = []) {
  if (!SHELL_LAUNCHERS.has(cmd)) return [];
  const lowerArgs = args.map((item) => String(item || "").toLowerCase());
  if (cmd === "cmd" || cmd === "cmd.exe") {
    const idx = lowerArgs.findIndex((item) => item === "/c" || item === "/k");
    if (idx >= 0) return materializeNestedTokens(args.slice(idx + 1));
    return [];
  }
  if (cmd === "powershell" || cmd === "powershell.exe" || cmd === "pwsh" || cmd === "pwsh.exe") {
    const idx = lowerArgs.findIndex((item) => item === "-command" || item === "-c" || item === "/c");
    if (idx >= 0) return materializeNestedTokens(args.slice(idx + 1));
    return [];
  }
  const idx = lowerArgs.findIndex((item) => item === "-c" || item === "-lc" || item === "/c");
  if (idx >= 0) return materializeNestedTokens(args.slice(idx + 1));
  return [];
}

function isDeleteLikeMoveDestination(rawToken = "", resolvedPath = "") {
  const raw = String(rawToken || "").trim().toLowerCase();
  if (!raw) return false;
  if (raw === "nul" || raw === "/dev/null") return true;
  if (/(?:^|[\\/])(?:\$recycle\.bin|\.trash)(?:[\\/]|$)/i.test(raw)) return true;
  if (/(?:^|[\\/])trash(?:[\\/]|$)/i.test(raw)) return true;
  const resolved = String(resolvedPath || "").trim().toLowerCase();
  if (!resolved) return false;
  if (/(?:^|[\\/])(?:\$recycle\.bin|\.trash)(?:[\\/]|$)/i.test(resolved)) return true;
  return /(?:^|[\\/])trash(?:[\\/]|$)/i.test(resolved);
}

function isWriteSink(rawToken = "", resolvedPath = "") {
  const raw = String(rawToken || "").trim().toLowerCase();
  if (!raw) return true;
  if (raw === "nul" || raw === "/dev/null" || raw === "-") return true;
  if (raw.startsWith("&")) return true;
  const resolved = String(resolvedPath || "").trim().toLowerCase();
  return resolved === "/dev/null";
}

function collectDeleteTargetsFromMoveCommand(cmd = "", args = [], cwd = process.cwd()) {
  const out = [];
  let endOfOptions = false;
  const positionals = [];
  for (const tok of args) {
    if (!tok) continue;
    if (!endOfOptions && tok === "--") {
      endOfOptions = true;
      continue;
    }
    if (!endOfOptions && isOptionTokenForCommand(cmd, tok)) continue;
    if (isRedirectionToken(tok)) continue;
    positionals.push(tok);
  }
  if (positionals.length < 2) return out;
  const rawDestination = positionals[positionals.length - 1];
  const destination = resolveCommandPath(rawDestination, cwd);
  if (!isDeleteLikeMoveDestination(rawDestination, destination)) return out;
  for (const src of positionals.slice(0, -1)) {
    const abs = resolveCommandPath(src, cwd);
    if (abs) out.push(abs);
  }
  return out;
}

function collectDeleteTargetsFromOsaScript(args = [], cwd = process.cwd()) {
  const scriptChunks = [];
  for (let i = 0; i < args.length; i += 1) {
    const tok = String(args[i] || "");
    if (!tok) continue;
    const lower = tok.toLowerCase();
    if ((lower === "-e" || lower === "-s" || lower === "-l") && i + 1 < args.length) {
      if (lower === "-e") {
        scriptChunks.push(String(args[i + 1] || ""));
      }
      i += 1;
      continue;
    }
    if (!tok.startsWith("-")) scriptChunks.push(tok);
  }

  if (scriptChunks.length === 0) return [];
  const script = scriptChunks.join("\n");
  const hasFinderContext = /\btell\s+app(?:lication)?\s+["']?finder["']?/i.test(script);
  if (!hasFinderContext) return [];

  const out = [];
  const patterns = [
    /\bdelete\s+posix\s+file\s+["']([^"']+)["']/gi,
    /\bmove\s+posix\s+file\s+["']([^"']+)["']\s+to\s+trash\b/gi,
  ];
  for (const pattern of patterns) {
    let match = null;
    while ((match = pattern.exec(script)) !== null) {
      const target = resolveCommandPath(match[1], cwd);
      if (target) out.push(target);
    }
  }
  return [...new Set(out)];
}

function collectDeleteTargetsFromCommand(tokens, cwd) {
  if (!tokens.length) return [];

  let idx = 0;
  while (idx < tokens.length && looksLikeAssignment(tokens[idx])) idx += 1;
  while (idx < tokens.length && COMMAND_WRAPPERS.has(normalizeCommandName(tokens[idx]))) idx += 1;
  if (idx >= tokens.length) return [];

  const cmd = normalizeCommandName(tokens[idx]);
  const args = tokens.slice(idx + 1);
  const nestedTokens = extractNestedCommandTokens(cmd, args);
  if (nestedTokens.length > 0) {
    return collectDeleteTargetsFromCommand(nestedTokens, cwd);
  }

  if (cmd === "find") {
    return collectDeleteTargetsFromFind(args, cwd);
  }

  if (MOVE_COMMANDS.has(cmd)) {
    return collectDeleteTargetsFromMoveCommand(cmd, args, cwd);
  }

  if (cmd === "osascript") {
    return collectDeleteTargetsFromOsaScript(args, cwd);
  }

  if (!DELETE_COMMANDS.has(cmd)) return [];

  const out = [];
  let endOfOptions = false;
  for (const tok of args) {
    if (!tok) continue;
    if (!endOfOptions && tok === "--") {
      endOfOptions = true;
      continue;
    }
    if (!endOfOptions && isOptionTokenForCommand(cmd, tok)) continue;
    if (isRedirectionToken(tok)) continue;
    const abs = resolveCommandPath(tok, cwd);
    if (abs) out.push(abs);
  }
  return out;
}

function resolveCd(segmentTokens, cwd) {
  let idx = 0;
  while (idx < segmentTokens.length && looksLikeAssignment(segmentTokens[idx])) idx += 1;
  while (idx < segmentTokens.length && COMMAND_WRAPPERS.has(normalizeCommandName(segmentTokens[idx]))) idx += 1;
  if (idx >= segmentTokens.length) return cwd;
  if (normalizeCommandName(segmentTokens[idx]) !== "cd") return cwd;

  const target = segmentTokens[idx + 1];
  if (!target || target === "-") return cwd;
  return resolveCommandPath(target, cwd) || cwd;
}

export function extractDeleteTargets(command, cwd) {
  const baseCwd = path.resolve(cwd || process.cwd());
  const tokens = tokenizeShell(command || "");
  const targets = [];
  let segment = [];
  let effectiveCwd = baseCwd;

  const flush = (separator) => {
    if (!segment.length) return;
    targets.push(...collectDeleteTargetsFromCommand(segment, effectiveCwd));
    if (separator !== "|") {
      effectiveCwd = resolveCd(segment, effectiveCwd);
    }
    segment = [];
  };

  for (const tok of tokens) {
    if (SEGMENT_BREAKS.has(tok)) {
      flush(tok);
      continue;
    }
    segment.push(tok);
  }
  flush(null);

  return [...new Set(targets.map((p) => path.resolve(p)))];
}

export function extractGuardPaths(command, cwd) {
  const baseCwd = path.resolve(cwd || process.cwd());
  const tokens = tokenizeShell(command || "");
  const targets = [];
  let segment = [];
  let effectiveCwd = baseCwd;

  const flush = (separator) => {
    if (!segment.length) return;
    const cmdIdx = getCommandTokenIndex(segment);
    for (let i = 0; i < segment.length; i += 1) {
      const tok = segment[i];
      if (!tok || i === cmdIdx) continue; // 命令本身（例如 /bin/ls）不是访问目标
      if (!isLikelyPathToken(tok)) continue;
      const abs = resolveCommandPath(tok, effectiveCwd);
      if (abs) targets.push(path.resolve(abs));
    }
    if (separator !== "|") {
      effectiveCwd = resolveCd(segment, effectiveCwd);
    }
    segment = [];
  };

  for (const tok of tokens) {
    if (SEGMENT_BREAKS.has(tok)) {
      flush(tok);
      continue;
    }
    segment.push(tok);
  }
  flush(null);

  return [...new Set(targets)];
}

function collectWriteTargetsFromTee(args = [], cwd = process.cwd()) {
  const out = [];
  let endOfOptions = false;
  for (const tok of args) {
    if (!tok) continue;
    if (!endOfOptions && tok === "--") {
      endOfOptions = true;
      continue;
    }
    if (!endOfOptions && tok.startsWith("-")) continue;
    if (isRedirectionToken(tok)) continue;
    const abs = resolveCommandPath(tok, cwd);
    if (!abs || isWriteSink(tok, abs)) continue;
    out.push(abs);
  }
  return out;
}

function collectWriteTargetsFromCopyLikeCommand(args = [], cwd = process.cwd()) {
  let endOfOptions = false;
  const positionals = [];
  for (const tok of args) {
    if (!tok) continue;
    if (!endOfOptions && tok === "--") {
      endOfOptions = true;
      continue;
    }
    if (!endOfOptions && tok.startsWith("-")) continue;
    if (isRedirectionToken(tok)) continue;
    positionals.push(tok);
  }
  if (positionals.length < 2) return [];
  const rawDestination = positionals[positionals.length - 1];
  const destination = resolveCommandPath(rawDestination, cwd);
  if (!destination || isWriteSink(rawDestination, destination)) return [];
  return [destination];
}

function collectWriteTargetsFromRedirection(tokens = [], cwd = process.cwd()) {
  const out = [];
  const prefixes = ["1>>", "2>>", ">>", "1>", "2>", ">"];
  for (let i = 0; i < tokens.length; i += 1) {
    const tok = String(tokens[i] || "");
    if (!tok) continue;

    let rawTarget = null;
    if (prefixes.includes(tok)) {
      rawTarget = String(tokens[i + 1] || "");
      i += 1;
    } else {
      const prefix = prefixes.find((item) => tok.startsWith(item) && tok.length > item.length);
      if (!prefix) continue;
      rawTarget = tok.slice(prefix.length);
    }

    if (!rawTarget) continue;
    const target = resolveCommandPath(rawTarget, cwd);
    if (!target || isWriteSink(rawTarget, target)) continue;
    out.push(target);
  }
  return out;
}

function collectWriteTargetsFromCommand(tokens, cwd) {
  if (!tokens.length) return [];

  let idx = 0;
  while (idx < tokens.length && looksLikeAssignment(tokens[idx])) idx += 1;
  while (idx < tokens.length && COMMAND_WRAPPERS.has(normalizeCommandName(tokens[idx]))) idx += 1;
  if (idx >= tokens.length) return [];

  const cmd = normalizeCommandName(tokens[idx]);
  const args = tokens.slice(idx + 1);
  const nestedTokens = extractNestedCommandTokens(cmd, args);
  if (nestedTokens.length > 0) {
    return collectWriteTargetsFromCommand(nestedTokens, cwd);
  }

  const out = [...collectWriteTargetsFromRedirection(tokens, cwd)];
  if (TEE_COMMANDS.has(cmd)) {
    out.push(...collectWriteTargetsFromTee(args, cwd));
  }
  if (MOVE_COMMANDS.has(cmd) || COPY_COMMANDS.has(cmd)) {
    out.push(...collectWriteTargetsFromCopyLikeCommand(args, cwd));
  }
  return out;
}

export function extractWriteTargets(command, cwd) {
  const baseCwd = path.resolve(cwd || process.cwd());
  const tokens = tokenizeShell(command || "");
  const targets = [];
  let segment = [];
  let effectiveCwd = baseCwd;

  const flush = (separator) => {
    if (!segment.length) return;
    targets.push(...collectWriteTargetsFromCommand(segment, effectiveCwd));
    if (separator !== "|") {
      effectiveCwd = resolveCd(segment, effectiveCwd);
    }
    segment = [];
  };

  for (const tok of tokens) {
    if (SEGMENT_BREAKS.has(tok)) {
      flush(tok);
      continue;
    }
    segment.push(tok);
  }
  flush(null);

  return [...new Set(targets.map((p) => path.resolve(p)))];
}

/**
 * 包装路径类工具（read, write, edit, grep, find, ls）
 */
export function wrapPathTool(tool, guard, operation, cwd) {
  return {
    ...tool,
    execute: async (toolCallId, params, ...rest) => {
      const rawPath = params.path;
      const absolutePath = resolvePath(rawPath, cwd);
      const checkPath = absolutePath || cwd;
      const result = guard.check(checkPath, operation);

      if (!result.allowed) {
        return blockedResult(result.reason);
      }

      return tool.execute(toolCallId, params, ...rest);
    },
  };
}

/**
 * 包装 bash 工具
 *
 * 1. preflight：常见危险命令提前拦截
 * 2. 路径校验：提取命令中的绝对路径，用 PathGuard 检查（Windows 无 OS 沙盒时的安全层）
 * 3. 执行：OS 沙盒在 BashOperations.exec 里生效（macOS/Linux）
 * 4. 错误翻译：OS 沙盒拦截后 stderr 的 Operation not permitted
 *
 * @param {object} tool  原始 bash 工具
 * @param {object} [guard]  PathGuard 实例（Windows 必传，macOS/Linux 可选）
 * @param {string} [cwd]  工作目录
 */
export function wrapBashTool(tool, guard, cwd) {
  return {
    ...tool,
    execute: async (toolCallId, params, ...rest) => {
      // preflight
      for (const [pattern, reasonFn] of PREFLIGHT_PATTERNS) {
        if (pattern.test(params.command)) {
          return blockedResult(reasonFn());
        }
      }

      // 路径校验：从命令中提取绝对路径，检查 PathGuard
      if (guard && cwd) {
        const deleteTargets = extractDeleteTargets(params.command, cwd);
        for (const p of deleteTargets) {
          const result = guard.check(p, "delete");
          if (!result.allowed) {
            return blockedResult(result.reason || t("sandbox.restrictedPath", { path: p }));
          }
        }

        const writeTargets = extractWriteTargets(params.command, cwd);
        for (const p of writeTargets) {
          const result = guard.check(p, "write");
          if (!result.allowed) {
            return blockedResult(result.reason || t("sandbox.restrictedPath", { path: p }));
          }
        }

        // 对 bash 命令中显式出现的路径做统一读取边界校验（全平台）。
        // 这是 OS 沙盒之外的兜底层，避免通过 cd/相对路径绕开边界提示。
        const guardPaths = extractGuardPaths(params.command, cwd);
        for (const p of guardPaths) {
          const result = guard.check(p, "read");
          if (!result.allowed) {
            return blockedResult(t("sandbox.restrictedPath", { path: p }));
          }
        }
      }

      try {
        const result = await tool.execute(toolCallId, params, ...rest);

        // 成功路径的错误翻译（exitCode 0 但 stderr 有 sandbox 拒绝）
        const text = result?.content?.[0]?.text;
        if (text && text.includes("Operation not permitted")) {
          result.content[0].text += "\n\n" + t("sandbox.writeRestricted");
        }

        return result;
      } catch (err) {
        // 兼容层对非零退出会 throw Error，错误消息里包含 stderr 输出。
        // 如果是沙盒拦截导致的，追加友好提示。
        if (err.message?.includes("Operation not permitted")) {
          err.message += "\n\n" + t("sandbox.writeRestricted");
        }
        throw err;
      }
    },
  };
}
