...
import {
  BLOCKED_FILES,
  BLOCKED_DIRS,
  PROJECTION_ONLY_AGENT_FILES,
  PROJECTION_ONLY_AGENT_GLOBS,
  PROJECTION_ONLY_HOME_FILES,
...
export class PathGuard {
   ...
   */
  constructor(policy = {}) {
    this._fullAccess = policy.mode === "full-access";
    this.mode = policy.mode === "balanced" ? "balanced" : "standard";
    this.hanakoHome = resolvePolicyPath(policy.hanakoHome);
    this.agentDir = resolvePolicyPath(policy.agentDir);
    this.workspace = resolvePolicyPath(policy.workspace);
    this.desktopPath = (() => {
  ...
  _getProjectionOnlyConflict(rawPath, operation) {
    if (operation !== "write" && operation !== "delete") return null;
    const resolved = this._resolveReal(rawPath);
    if (!resolved) return null;

    const exactProtectedPaths = [];
    if (this.agentDir) {
      for (const rel of PROJECTION_ONLY_AGENT_FILES) {
        exactProtectedPaths.push(path.join(this.agentDir, rel));
      }
      for (const rel of PROJECTION_ONLY_AGENT_GLOBS) {
        if (!rel.includes("*")) exactProtectedPaths.push(path.join(this.agentDir, rel));
      }
    }
    if (this.hanakoHome) {
      for (const rel of PROJECTION_ONLY_HOME_FILES) {
        exactProtectedPaths.push(path.join(this.hanakoHome, rel));
      }
    }

    for (const protectedPath of exactProtectedPaths) {
      if (resolved === protectedPath) return protectedPath;
      if (operation === "delete" && this._isInside(protectedPath, resolved)) return protectedPath;
    }

    if (this.agentDir && PROJECTION_ONLY_AGENT_GLOBS.includes("experience/*.md")) {
      const experienceDir = path.join(this.agentDir, "experience");
      const isExperienceTarget = resolved === experienceDir || this._isInside(resolved, experienceDir);
      if (isExperienceTarget) {
        if (operation === "delete") return resolved;
  ...
  getAccessLevel(rawPath) {
    ...
    if (this.mode === "balanced" && this.agentDir && this._isInside(resolved, path.join(this.agentDir, "skills"))) {
      return AccessLevel.READ_WRITE;
    }
    ...
    if (this.hanakoHome && this._isInside(resolved, this.hanakoHome)) return AccessLevel.BLOCKED;
  ...
  check(absolutePath, operation) {
    const projectionConflict = this._getProjectionOnlyConflict(absolutePath, operation);
    if (projectionConflict) {
      ...
      return {
        allowed: false,
        reason: t("sandbox.denied", { op: opLabel, path: resolved, level: AccessLevel.READ_ONLY }),
      };
    }
