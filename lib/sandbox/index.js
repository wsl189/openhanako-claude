 ...
 */
export function createSandboxedTools(cwd, customTools, { agentDir, workspace, hanakoHome, mode, pathRules = [] }) {
  const policy = deriveSandboxPolicy({ agentDir, workspace, hanakoHome, mode, pathRules });
  ...
  // strict/balanced: PathGuard + OS 沙盒 exec（balanced 仅放开 bash 网络）
  ...
  return {
    tools: [
      ...
      wrapPathTool(createWriteTool(cwd), guard, "write", cwd),
      wrapPathTool(createEditTool(cwd), guard, "write", cwd),
      wrapBashTool(createBashTool(cwd, { operations: bashOps }), guard, cwd),
      wrapPathTool(createGrepTool(cwd), guard, "read", cwd),
      wrapPathTool(createFindTool(cwd), guard, "read", cwd),
      wrapPathTool(createLsTool(cwd), guard, "read", cwd),
    ],
